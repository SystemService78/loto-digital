...
# --- 1. CONEXÃO ---
# 1. Transformamos os segredos em um dicionário comum para podermos editar
gs_secrets = st.secrets["connections"]["gsheets"].to_dict()

# 2. Limpamos a chave privada (corrige as barras e quebras de linha)
if "private_key" in gs_secrets:
    gs_secrets["private_key"] = gs_secrets["private_key"].replace("\\n", "\n")

# 3. Importante: removemos o "type" do dicionário para não dar aquele 
# erro de "multiple values for argument 'type'" que vimos antes
if "type" in gs_secrets:
    del gs_secrets["type"]

# 4. Agora sim, iniciamos a conexão passando o dicionário corrigido
