...
try:
    # 1. Pegamos os dados da seção 'minha_planilha' que você criou
    # Usamos o .copy() para garantir que não estamos mexendo no original
    gs_info = st.secrets["minha_planilha"].to_dict().copy()
    
    # 2. Corrigimos a chave privada (remove as barras extras do Windows/OneDrive)
    if "private_key" in gs_info:
        gs_info["private_key"] = gs_info["private_key"].replace("\\n", "\n")
    
    # 3. Extraímos a URL para uma variável e REMOVEMOS do dicionário
    url_final = gs_info.pop("spreadsheet", None)

    # 4. AQUI ESTÁ A SOLUÇÃO: Removemos o 'type' do Google (service_account)
    # para ele não brigar com o 'type' do Streamlit (GSheetsConnection)
    if "type" in gs_info:
        gs_info.pop("type")
    
    # 5. Criamos a conexão. 
    # Mudamos o nome de "gsheets" para "loto_db" para forçar o Streamlit a esquecer erros antigos.
...
def executar_sql(query, params=()):
    """Simula UPDATE e DELETE lendo, filtrando e salvando de volta"""
    query_up = query.upper()
    
    if "DELETE FROM ATIVOS_LOTO" in query_up:
...
def ler_sql(query, params=()):
    ...
    try:
        # Leitura limpa, confiando no config do servidor
        ...
        if "WHERE" in query_up and params:
            coluna = query_up.split("WHERE")[1].split("=")[0].strip()
            df_gs = df_gs[df_gs[coluna].astype(str).str.lower() == str(params[0]).lower()]
            
        return df_gs
    except Exception as e:
        # Print aparece nos logs do servidor Streamlit
        print(f"Erro na leitura da aba {tabela_alvo}: {e}")
        return pd.DataFrame()
