...
st.markdown("""
    <style>
    /* Estilo do Card Branco (Visual Original) */
    .metric-container {
        ...
        height: 200px; /* Aumentado levemente para caber o botão */
        display: flex;
            ...
            with st.expander("❓ Esqueci minha senha"):
                ...
                if st.button("Enviar Solicitação", use_container_width=True):
                    ...
                    if email_in and f_nome:
                        agora_sol = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=-3))).strftime("%d/%m/%Y %H:%M")
...
with st.sidebar:
    ...
    st.markdown("### Menu")
    idx_atual = OPCOES_MENU.index(st.session_state.pagina_ativa) if st.session_state.pagina_ativa in OPCOES_MENU else 0
...
if st.session_state.pagina_ativa == "🏠 Início":
    ...
    agora_br = datetime.datetime.now(fuso_brasil)
    hora_agora = agora_br.hour
        ...
        if ativos_usuario:
            ...
            for reg_at in ativos_usuario:
                ...
                try:
                    ...
                    h_ini_str = info_loto.get('HORA_INICIO', '')
                ...
                if progresso_h >= 7:
                    st.error(f"🚨 **ALERTA DE SEGURANÇA:** A máquina **{maq_at}** está FORA DE OPERAÇÃO há **{tempo_formatado}**.")
                ...
                if info_loto.get('TIPO') == 'SESMT':
                    st.markdown(f"""
                        <div style="background-color: #FEE2E2; padding: 15px; border-radius: 10px; border-left: 5px solid #EF4444; margin-bottom: 10px;">
                            ...
                            <span style="color: #B91C1C;">Tempo de Interdição: <b>{tempo_formatado}</b></span>
                        </div>
                    """, unsafe_allow_html=True)
...
##trecho removido daqui
    ...
    with st.container(border=True):
        ...
        if setor_sel and setor_sel != "Selecione o setor":
            ...
            # 1. Busca se existe algum registro ativo desta máquina feito pelo SESMT
            ...
            if res_sesmt.data:
                for reg in res_sesmt.data:
                    ...
                    if d and str(d.get('EQUIP')).upper() == str(maquina_sel).upper():
                        if d.get('TIPO') == 'SESMT': # Se o tipo gravado foi SESMT
                            interditada = True
                            dados_interdicao = d
                            break

            # 2. Se estiver interditada, bloqueia tudo
            if interditada:
                st.error(f"🚫 **MÁQUINA INTERDITADA PELO SESMT**")
                st.warning(f"Esta máquina foi suspensa por: **{dados_interdicao.get('RESP')}**")
                st.info("Motivo: Verifique as condições de segurança antes de prosseguir. A execução está bloqueada até a liberação do SESMT.")
                
                # Se o usuário logado NÃO FOR o próprio SESMT, ele não vê o botão de continuar
                if st.session_state.usuario['PERFIL'] != 'SESMT':
                    if st.button("⬅️ Voltar ao Início"):
                        navegar_para("🏠 Início")
                    st.stop() # Mata o processamento da página aqui

    # --- 4. PASSO 2: IDENTIFICAÇÃO ---
    if maquina_sel:
        if st.session_state.hora_inicio_loto is None:
            st.session_state.hora_inicio_loto = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=-3))).strftime("%H:%M:%S")

        with st.container(border=True):
            st.subheader("📝 Passo 2: Identificação")
            
            # Pega a área do cadastro do usuário
            area_usuario = st.session_state.usuario.get('AREA', 'Operação')
            
            # Exibe a área mas não permite alteração (disabled)
            st.radio(
                "Sua Área de Atuação:", 
                ["Manutenção Mecânica", "Manutenção Elétrica", "Operação", "SESMT"], 
                index=["Manutenção Mecânica", "Manutenção Elétrica", "Operação", "SESMT"].index(area_usuario) if area_usuario in ["Manutenção Mecânica", "Manutenção Elétrica", "Operação", "SESMT"] else 2,
                horizontal=True, 
                key="tipo_exec_display",
                disabled=True # <--- TRAVA AQUI
            )
            # Garante que o valor usado internamente no código seja o da área fixa do usuário
            tipo = area_usuario
                        ...
                        if st.button("🏁 Finalizar e Registrar", type="primary", use_container_width=True, key="btn_finalizar_unico"):
                            ...
                            if foto:
                                try:
                                    ...
                                    agora_str = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=-3))).strftime('%Y%m%d_%H%M%S')
...
# --- 7. HISTÓRICO E DETALHAMENTO ---
    ...
    if not df_hist.empty:
        ...
        def renderizar_card_na_lista(indice, dados_linha, aba):
            ...
            info_extra = f" | 👤 {dados_linha['RESP']}" if user_atual['PERFIL'] == 'Perfil System' else ""
...
# --- 8. MEU PERFIL ---
    ...
    with st.container():
        ...
        with col_p2:
            ...
            todas_areas = [
                "Manutenção Mecânica", "Manutenção Elétrica", "Operação", 
                "SESMT", "Supervisão Manutenção", "TI", "Diretoria", "Administrador Master"
            ]
            
            # Garante que a área atual do usuário esteja na lista para não dar erro de index
            area_atual = user.get('AREA', 'Operação')
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        if user_p['PERFIL'] == 'Perfil System':
            ...
            # Admin Cliente (Perfil Gerencial Clientes) vê apenas duas
        ...
        if user_p['PERFIL'] == 'Perfil System':
            with tab_cadastro:
                if user_p['PERFIL'] == 'Perfil System': # Proteção extra
                    ...
                    with st.container(border=True):
                        ...
                        # Busca empresas existentes no banco para facilitar a vida do Perfil System
                        df_u_temp = ler_sql("usuarios")
                        lista_empresas_existentes = sorted(list(df_u_temp['EMPRESA'].unique())) if not df_u_temp.empty else []

                        if user_logado['PERFIL'] == 'Perfil System':
                            c_emp1, c_emp2 = st.columns(2)
                            modo_empresa = c_emp1.selectbox("Empresa:", ["Selecionar Existente", "Cadastrar Nova"])
                            
                            if modo_empresa == "Selecionar Existente":
                                empresa_escolhida = c_emp2.selectbox("Lista de Empresas:", lista_empresas_existentes)
                                st.session_state["reg_empresa"] = empresa_escolhida
                            else:
                                empresa_digitada = c_emp2.text_input("Nome da Nova Empresa:", placeholder="Digite o nome aqui...")
                                st.session_state["reg_empresa"] = empresa_digitada
                        else:
                            # Perfil Gerencial Clientes: Trava na empresa dele
                            st.text_input("Empresa", value=user_logado['EMPRESA'], key="reg_empresa_view", disabled=True)
                            st.session_state["reg_empresa"] = user_logado['EMPRESA']
                        ...
                        # --- NOVO BLOCO DE PERFIL E ÁREA (DEPENDENTES) ---
                        c1, c2 = st.columns(2)
                        
                        # 1. Seleciona o Perfil primeiro (na primeira coluna agora)
                        perfil_selecionado = c1.selectbox(
                            "Nível de Acesso (Perfil) *", 
                            ["Perfil Operação", "Perfil Gerencial Clientes", "Perfil System"]
                        )

                        # 2. Define as áreas baseadas no perfil escolhido
                        if perfil_selecionado == "Perfil Operação":
                            opcoes_area = ["Manutenção Mecânica", "Manutenção Elétrica", "SESMT"]
                        elif perfil_selecionado == "Perfil Gerencial Clientes":
                            opcoes_area = ["Supervisão Manutenção"]
                        else: # Perfil System
                            opcoes_area = ["Administrador Master"]

                        # 3. Exibe a Área (na segunda coluna agora)
                        area_selecionada = c2.selectbox("Área de Atuação *", opcoes_area)
                        
                        st.session_state["reg_perfil"] = perfil_selecionado
                        st.session_state["reg_area"] = area_selecionada
                ...
                if not df_u.empty:
                    ...
                    if selecao:
                        ...
                        with st.container(border=True):
                            ...
                            novo_perfil = c_ed2.selectbox("Nível de Acesso", ["Perfil Operação", "Perfil System", "Perfil Gerencial Clientes"],
                                                        index=["Perfil Operação", "Perfil System", "Perfil Gerencial Clientes"].index(str(dados_u['PERFIL'])))
                            
                            # AJUSTE: Campo para editar a Empresa (Para evitar o 'None' no futuro)
