...
st.markdown("""
    ...
    /* 1. FORCE LIGHT THEME VARIABLES (Sobrescreve o motor do Streamlit) */
    :root {
    ...
    /* 2. BLINDAGEM DO FUNDO E TEXTO */
    .stApp, [data-testid="stAppViewContainer"], [data-testid="stHeader"], [data-testid="stSidebar"], .stAppViewMain {
        background-color: white !important;
        color: #1E293B !important;
    }

    /* 3. FIX PARA INPUTS, SELETORES E EXPANDERS (REMOVER O PRETO) */
    ...
    /* Texto dentro dos seletores e menus */
    div[data-baseweb="popover"] div { color: #1E293B !important; }
    
    /* 4. CORRIGIR AS CORES DAS MÉTRICAS (GARANTIA) */
    .metric-label-custom { color: #64748B !important; font-weight: 600 !important; }
    .card-total .metric-value-custom { color: #1E3A8A !important; font-weight: 800 !important; }
    .card-exec .metric-value-custom { color: #3B82F6 !important; font-weight: 800 !important; }
    .card-conc .metric-value-custom { color: #10B981 !important; font-weight: 800 !important; }
    .card-canc .metric-value-custom { color: #EF4444 !important; font-weight: 800 !important; }

    /* 5. SIDEBAR - FUNDO E SELEÇÃO */
    [data-testid="stSidebar"] { background-color: #F8FAFC !important; border-right: 1px solid #E2E8F0; }
    [data-testid="stSidebar"] [data-testid="stMarkdownContainer"] p { color: #1E293B !important; }
    
    /* 6. TRANSPARÊNCIA DE PNG */
    img, [data-testid="stImage"] { background-color: transparent !important; mix-blend-mode: multiply; }

    /* 7. ALERTA VIBRANTE */
    .alerta-vermelho {
        background-color: #FEE2E2 !important;
        color: #991B1B !important;
        padding: 15px; border-radius: 10px; border-left: 6px solid #EF4444; margin-bottom: 10px; font-weight: bold;
    }
    ...
    /* Estilo para centralizar o botão na coluna e dar o aspecto de pill */
    div.stButton {
        text-align: center; /* Centraliza o container do botão */
    }
    ...
    div.stButton > button {
        ...
        padding: 0 20px !important; /* Dá largura interna ao botão */
                ...
                if progresso_h >= 7:
                    st.markdown(f"""
                        <div class="alerta-vermelho">
                            🚨 ALERTA DE SEGURANÇA: A máquina {maq_at} está FORA DE OPERAÇÃO há {tempo_formatado}.
                        </div>
                    """, unsafe_allow_html=True)
            ...
            if df_visualizacao.empty:
                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        # --- CÁLCULO DE TEMPO (OBJETIVO E DEFINITIVO) ---
                        tempo_formatado = "---"
                        try:
                            fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
                            # Pegamos o agora JÁ no fuso e removemos a etiqueta para conta pura
                            agora_br = datetime.datetime.now(fuso_br).replace(tzinfo=None)
                            ...
                            if h_ini_str:
                                ...
                                # Se o início gravado ficou maior que agora (fuso/virada), recua 1 dia
                                if dt_ini > agora_br:
                    ...
                    with st.container(border=True):
                        ...
                        with c3:
                            ...
                            if st.button("Marcar como resolvido ✅", key=f"btn_sol_{row['ID']}", use_container_width=True):
                                ...
                                # Marcamos que queremos voltar para a aba de pedidos (usando o ícone como âncora)
                                st.session_state.aba_admin_ativa = "🔑 Pedidos de Recuperação" 
                                st.rerun()
