...
st.markdown("""
    ...
    /* Estilo base para cada item do checklist (Delineado) */
    /* Remove as margens padrão do Streamlit para colar os itens */
    [data-testid="stVerticalBlock"] > div:has(.step-card-anchor) {
        gap: 0 !important;
    }

    /* Estilização para o container com borda do Streamlit */
    /* Quando não concluído */
    div[data-testid="stElementContainer"]:has(.step-card-anchor) + div[data-testid="stVerticalBlockBorderWrapper"] > div {
    ...
    /* Quando concluído (Verde) */
    div[data-testid="stElementContainer"]:has(.step-card-complete-anchor) + div[data-testid="stVerticalBlockBorderWrapper"] > div {
    ...
    .hist-status { 
        ...
        color: white !important; 
        padding: 4px 12px !important; border-radius: 15px !important; font-size: 0.8rem !important; font-weight: bold !important;
    }
    .hist-sub { color: #64748B !important; font-size: 0.9rem !important; margin-top: 4px !important; }

    .hist-status-cancel { 
        background-color: #DC2626 !important; /* Vermelho mais forte */
        color: white !important; 
        padding: 4px 12px !important; border-radius: 15px !important; font-size: 0.8rem !important; font-weight: bold !important;
    }
    ...
    try:
        ...
        # --- CASO C: ATUALIZAÇÃO (UPDATE) ---
            ...
            if tabela_alvo == "solicitacoes":
                ...
                # NOVO: Se a query for de renovação de licença (identificada pelo WHERE EMPRESA)
                elif "WHERE EMPRESA =" in query_up.upper():
                    # Usa apenas o primeiro parâmetro (empresa) e ignora o resto
                    conn.table("usuarios").update({"ativo": "Sim"}).eq("empresa", params[0]).execute()
                
                # CASO PADRÃO: Edição de perfil (usa os 6 parâmetros originais)
                else:
...
if 'pasta_atual' not in st.session_state: st.session_state.pasta_atual = ""
if 'temp_galeria' not in st.session_state: st.session_state.temp_galeria = []
...
def realizar_login(email, senha):
    ...
    # Verifica licença da empresa antes de validar o usuário
    # Busca dados básicos para verificar se é System ou se a licença expirou
    df_u_temp = ler_sql(f"SELECT EMPRESA, PERFIL FROM usuarios WHERE EMAIL = '{email.strip().lower()}'")
    
    if not df_u_temp.empty:
        dados_temp = df_u_temp.iloc[0]
        emp_check = dados_temp['EMPRESA']
        perfil_check = dados_temp['PERFIL']

        # SÓ VERIFICA LICENÇA SE NÃO FOR PERFIL SYSTEM
        if perfil_check != 'Perfil System':
            res_l = conn.table("licencas").select("data_vencimento").eq("empresa", emp_check).execute()
            if res_l.data:
                dv = datetime.datetime.strptime(res_l.data[0]['data_vencimento'], "%Y-%m-%d").date()
                if dv < datetime.date.today():
                    # SE EXPIROU: Bloqueia todos os usuários daquela empresa
                    executar_sql("UPDATE usuarios SET ATIVO = 'Não' WHERE EMPRESA = ?", (emp_check,))
...
def gerar_pdf_loto_premium(dados_os, lista_itens):
    ...
    obs_texto = str(dados_os.get('OBS_GERAL', ''))
    if "Fotos: " in obs_texto:
        try:
            caminho_primeira_foto = obs_texto.split("Fotos: ")[-1].split(", ")[0].strip()
            # Baixa a imagem do Supabase para o PDF
            response = requests.get(conn.client.storage.from_("evidencias").get_public_url(caminho_primeira_foto))
            if response.status_code == 200:
                img_data = BytesIO(response.content)
                # Coordenadas: x=145 (direita), y=235 (baixo), largura=50mm
                pdf.image(img_data, x=145, y=235, w=50)
        except:
            pass # Se falhar a foto, o PDF sai sem ela mas não trava
...
with st.sidebar:
    ...
    if user_p['PERFIL'] == 'Perfil System':
        OPCOES_MENU = ["🏠 Início", "➕ Nova Execução", "📊 Visão Geral", "⚙️ Painel Admin", "📜 Licença", "🛠️ Suporte"]
    elif user_p['PERFIL'] == 'Perfil Gerencial Clientes':
        OPCOES_MENU = ["🏠 Início", "➕ Nova Execução", "📊 Visão Geral", "⚙️ Painel Admin", "📜 Licença", "🛠️ Suporte"]
    else:
        OPCOES_MENU = ["🏠 Início", "➕ Nova Execução", "📊 Visão Geral", "📜 Licença", "🛠️ Suporte"]
...
if st.session_state.pagina_ativa == "🏠 Início":
    ...
    # 1. Busca os dados do usuário logado para as validações abaixo
    user_atual = st.session_state.usuario
    user_nome = user_atual['NOME']
    perfil_logado = user_atual['PERFIL']

    # 2. Inicializa os contadores (antiga linha 821)
    ...
    lista_bruta = res_bruto.data if res_bruto.data else []
    
    ativos_usuario = []
    # --- 2. FILTRAGEM DE SEGURANÇA E VISIBILIDADE ---
    for reg in lista_bruta:
        info = reg.get('dados_json') or reg.get('DADOS_JSON')
        if not info: continue
        
        dono_do_registro = reg['resp'].split('|')[0]
        is_meu = dono_do_registro == user_nome
        is_sesmt = info.get('TIPO') == 'SESMT'
        mesma_empresa = info.get('EMPRESA') == user_atual.get('EMPRESA')

        # Regra: System vê tudo | Gerencial vê SESMT da empresa + os dele | Operador vê SESMT da empresa + os dele
        if perfil_logado == 'Perfil System':
            ativos_usuario.append(reg)
        elif is_meu or (is_sesmt and mesma_empresa):
            ativos_usuario.append(reg)

    # --- 3. ATUALIZAÇÃO DO CONTADOR DO CARD (Resolve o erro do 14) ---
    total_ativos = len(ativos_usuario)
    ...
    with c_b2:
        if ativos_usuario:
            ...
            for reg_at in ativos_usuario:
                ...
                # --- NOVA TRAVA DE EXIBIÇÃO ---
                is_meu_registro = reg_at['resp'].split('|')[0] == user_nome
                is_sesmt = info_loto.get('TIPO') == 'SESMT'
                
                # Se não for meu e não for SESMT, pula para o próximo (não mostra na lista)
                if not is_meu_registro and not is_sesmt:
                    continue
                ...
                try:
                    ...
                    # CORREÇÃO: Usar 'info_loto' em vez de 'row'
                    h_ini_str = str(info_loto.get('HORA_INICIO', ''))
                    
                    if h_ini_str and h_ini_str != 'None':
                        dt_ini = None
                        # Tenta os formatos (incluindo segundos)
                        for formato in ["%d/%m/%Y %H:%M:%S", "%d/%m/%Y %H:%M", "%H:%M:%S"]:
                            try:
                                dt_ini = datetime.datetime.strptime(h_ini_str, formato)
                                if formato == "%H:%M:%S":
                                    dt_ini = dt_ini.replace(year=agora_br.year, month=agora_br.month, day=agora_br.day)
                                break
                            except: continue
                ...
                if info_loto.get('TIPO') == 'SESMT':
                    # Pega o nome do responsável (ajustado para evitar None)
                    ...
                    st.markdown(f"""
                        <div style="background-color: #FEE2E2; padding: 15px; border-radius: 10px; border-left: 5px solid #EF4444; margin-bottom: 10px;">
                            ...
                            <span style="color: #B91C1C;">Duração do Bloqueio: <b>{tempo_formatado}</b></span><br>
    ...
    with st.container(border=True):
        ...
        if setor_sel and setor_sel != "Selecione o setor":
            ...
            if interditada:
                ...
                nome_responsavel = dados_interdicao.get('RESP') or dados_interdicao.get('resp') or (str(id_registro_interdicao).split('|')[0] if id_registro_interdicao else "Não identificado")
                ...
                if os_atual != os_banco:
                    ...
                    if dono == user_nome:
                        ...
                        if c1.button("▶️ Continuar Meu Trabalho", use_container_width=True, key="btn_ret_v10"):
                            # --- RESTAURAÇÃO INTEGRAL (FIX: Removido setor e maq exec) ---
                            # Note: Não alteramos setor_exec, maq_exec ou tipo_exec_display 
                            # pois os widgets já estão na tela com esses valores.
        ...
        if os_n:
            ...
            with st.container(border=True):
                ...
                for i, (icone, nome) in enumerate(itens_loto, 1):
                    ...
                    with st.container():
                # Cria a âncora para o CSS saber se pinta de verde ou cinza
                        ancora = "step-card-complete-anchor" if is_ch else "step-card-anchor"
                        st.markdown(f'<div class="{ancora}"></div>', unsafe_allow_html=True)

                        # Usamos border=True para o Streamlit criar o contorno real em volta de tudo
                        with st.container(border=True):
                ...
                st.markdown("---")
                st.subheader("📸 Evidências de Bloqueio")
                st.caption("Capture até 5 fotos. A primeira é recomendada.")

                if 'temp_galeria' not in st.session_state:
                    st.session_state.temp_galeria = []

                # Câmera única: ao tirar a foto, ela entra na lista abaixo
                if len(st.session_state.temp_galeria) < 5:
                    nova_foto = st.camera_input("Capturar Evidência (Clique para cada foto)", key="camera_unica")
                    if nova_foto:
                        # Evita duplicatas no clique
                        if nova_foto.name not in [f.name for f in st.session_state.temp_galeria]:
                            st.session_state.temp_galeria.append(nova_foto)
                            st.rerun()
                else:
                    st.warning("📸 Limite de 5 fotos atingido.")

                # Exibe as miniaturas das fotos já tiradas
                if st.session_state.temp_galeria:
                    cols_galeria = st.columns(5)
                    for idx, foto_salva in enumerate(st.session_state.temp_galeria):
                        with cols_galeria[idx]:
                            st.image(foto_salva, caption=f"Foto {idx+1}", use_container_width=True)
                    
                    if st.button("🗑️ Limpar Galeria", use_container_width=True):
                        st.session_state.temp_galeria = []
                        st.rerun()
                ...
                with col_f1:
                    if all(status_etapas):
                        ...
                        if st.button("🏁 Finalizar e Registrar", type="primary", use_container_width=True, key="btn_finalizar_unico"):
                            ...
                            nomes_fotos_salvas = []
                            # Verificamos a galeria temporária que criamos
                            if st.session_state.temp_galeria:
                                try:
                                    agora_f = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=-3)))
                                    agora_str_f = agora_f.strftime('%H%M%S')
                                    
                                    # Loop pelas fotos da galeria
                                    for idx, f_arq in enumerate(st.session_state.temp_galeria, 1):
                                    ...
                                    st.warning(f"Erro ao salvar fotos: {e}")

                            # 2. PREPARAÇÃO DOS DADOS
                            lista_fotos_str = ", ".join(nomes_fotos_salvas)
                            obs_final_com_fotos = f"{obs_final} | Fotos: {lista_fotos_str}" if nomes_fotos_salvas else obs_final
...
# --- 7. HISTÓRICO E DETALHAMENTO ---
    ...
    if not df_hist.empty:
        ...
        if st.session_state.id_os_detalhe is not None:
            ...
            # --- ADICIONADO: EXIBIÇÃO DE FOTOS NOS DETALHES ---
            st.markdown("### 📸 Evidências Fotográficas")
            obs_texto = str(row['OBS_GERAL'])
            if "Fotos: " in obs_texto:
                try:
                    # Extrai os caminhos das fotos do texto
                    lista_caminhos = obs_texto.split("Fotos: ")[-1].split(", ")
                    cols_fotos = st.columns(len(lista_caminhos) if len(lista_caminhos) <= 5 else 5)
                    for i, path in enumerate(lista_caminhos):
                        url_foto = conn.client.storage.from_("evidencias").get_public_url(path.strip())
                        with cols_fotos[i % 5]:
                            st.image(url_foto, use_container_width=True, caption=f"Foto {i+1}")
                except:
                    st.info("Caminho das fotos não localizado no servidor.")
            else:
                st.info("Nenhuma foto anexada a esta execução.")
            ...
            # 1. DEFINE AS OPÇÕES LOGO NO INÍCIO (ESSENCIAL PARA NÃO DAR ERRO)
            opcoes_status = ["Todas", "Ativas", "Concluídas", "Canceladas"]

            # 2. Se veio da Home, transfere a escolha para a memória oficial
            ...
            # 3. Garante que a gaveta de memória exista
            if 'aba_admin_ativa' not in st.session_state:
                st.session_state.aba_admin_ativa = "Todas"

            # 4. Seletor de pílulas (Segmented Control)
            escolha_status = st.segmented_control(
                "Filtrar visualização:", 
                opcoes_status, 
                default=st.session_state.aba_admin_ativa if st.session_state.aba_admin_ativa in opcoes_status else "Todas",
                selection_mode="single",
                key="radio_visao_geral"
            )
            
            # 5. Sincroniza a memória com o clique
            if escolha_status:
                st.session_state.aba_admin_ativa = escolha_status
            else:
                escolha_status = st.session_state.aba_admin_ativa
                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
                            agora_br = datetime.datetime.now(fuso_br).replace(tzinfo=None)
                            # CORREÇÃO: Usar 'row' em vez de 'info_loto'
                            h_ini_raw = str(row.get('HORA_INICIO', ''))
                            
                            if h_ini_raw and h_ini_raw != 'None':
                                dt_ini = None
                                # Tenta converter incluindo o novo formato com SEGUNDOS (%S)
                                for formato in ["%d/%m/%Y %H:%M:%S", "%d/%m/%Y %H:%M", "%H:%M:%S"]:
                                    try:
                                        dt_ini = datetime.datetime.strptime(h_ini_raw, formato)
                                        if formato == "%H:%M:%S":
                                            dt_ini = dt_ini.replace(year=agora_br.year, month=agora_br.month, day=agora_br.day)
                                        break
                                    except: continue
                                ...
                                if dt_ini:
                                    total_sec = abs(int((agora_br - dt_ini).total_seconds()))
                                    dias = total_sec // 86400
                                    horas = (total_sec % 86400) // 3600
                                    minutos = (total_sec % 3600) // 60
                                    tempo_formatado = f"{dias}d {horas}h {minutos}min" if dias > 0 else f"{horas}h {minutos}min"
                        except:
...
# --- 9. PÁGINA DE CONSULTA DE LICENÇA ---
elif st.session_state.pagina_ativa == "📜 Licença":
    st.title("📜 Status da sua Licença")
    
    empresa_user = st.session_state.usuario.get('EMPRESA', 'N/D')
    
    # Busca a data no banco de licenças
    res_lic = conn.table("licencas").select("data_vencimento").eq("empresa", empresa_user).execute()
    
    if res_lic.data:
        data_venc = datetime.datetime.strptime(res_lic.data[0]['data_vencimento'], "%Y-%m-%d").date()
        hoje = datetime.date.today()
        dias_restantes = (data_venc - hoje).days
        
        if dias_restantes > 0:
            st.success(f"Sua licença está **ATIVA**.")
            st.metric("Dias restantes de acesso", f"{dias_restantes} dias")
            st.progress(min(max(dias_restantes / 365, 0.0), 1.0))
            st.caption(f"Vencimento em: {data_venc.strftime('%d/%m/%Y')}")
        else:
            st.error(f"🚨 LICENÇA EXPIRADA (Há {abs(dias_restantes)} dias)")
            st.warning("O acesso às funções de execução foi bloqueado. Entre em contato com o suporte para renovar.")
    else:
        st.info("Esta empresa possui licença vitalícia ou não monitorada.")

# --- 10. PÁGINA DE SUPORTE ---
elif st.session_state.pagina_ativa == "🛠️ Suporte":
    st.title("🛠️ Central de Ajuda e Suporte")
    ...
    with st.container(border=True):
        ...
        with c2:
            st.markdown("### System Service - LOTO Digital")
            st.write("Versão Pro 2.0 | Licença Enterprise")
    ...
    with col1:
        ...
        st.info("**Telefone:** (81) 98943-2369\n\n**E-mail:** suporteloto@systemservice.net.br")
        
    with col2:
        st.subheader("💬 Mensagem")
        # Link direto para WhatsApp (Exemplo)
        st.link_button("Abrir WhatsApp Suporte", "https://wa.me/+5581989432369", use_container_width=True)
        st.caption("Atendimento de Seg. a Sex. - 08h às 18h")
        
    with col3:
        st.subheader("📚 Documentação")
        st.button("Manual do Usuário", use_container_width=True)
        #st.button("Normas Regulamentadoras", use_container_width=True)

    st.write("---")
    st.caption("Desenvolvido por System Service. Todos os direitos reservados.")
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        if user_p['PERFIL'] == 'Perfil System':
            opcoes_admin = ["➕ Novo Usuário", "📝 Editar Usuário", "🛠️ Equipamentos", "📜 Gestão Licenças", label_pedidos, "📁 Arquivos/Fotos"]
        else:
        ...
        elif escolha_aba_admin == "📜 Gestão Licenças":
            st.subheader("Controle de Licenciamento por Empresa")
            
            # Busca todas as empresas que possuem usuários cadastrados
            df_u_all = ler_sql("SELECT DISTINCT EMPRESA FROM usuarios")
            lista_emp = sorted([str(x) for x in df_u_all['EMPRESA'].unique() if x and str(x).lower() != 'none'])
            
            emp_sel_lic = st.selectbox("Selecione a Empresa para renovação:", lista_emp)
            
            with st.container(border=True):
                c1, c2 = st.columns(2)
                modo_lic = c1.radio("Forma de Renovação:", ["Por Período (30, 60, 90 dias)", "Data Específica"])
                
                if modo_lic == "Por Período (30, 60, 90 dias)":
                    dias = c2.selectbox("Quantidade de dias:", [30, 60, 90, 180, 365])
                    data_fim = datetime.date.today() + datetime.timedelta(days=dias)
                else:
                    data_fim = c2.date_input("Vencimento da Licença:", datetime.date.today() + datetime.timedelta(days=30))
                
                st.info(f"A licença da empresa **{emp_sel_lic}** será válida até: **{data_fim.strftime('%d/%m/%Y')}**")
                
                if st.button("🚀 Confirmar Renovação | Liberar Acesso", type="primary", use_container_width=True):
                    # Garantimos que a data vá como string no formato ISO (AAAA-MM-DD)
                    data_venc_iso = data_fim.strftime("%Y-%m-%d")
                    
                    # Executa o salvamento
                    conn.table("licencas").upsert({
                        "empresa": emp_sel_lic, 
                        "data_vencimento": data_venc_iso
                    }).execute()
                    
                    # Reativa os usuários
                    executar_sql("UPDATE usuarios SET ATIVO = 'Sim' WHERE EMPRESA = ?", (emp_sel_lic,))
                    
                    st.success(f"✅ Licença da {emp_sel_lic} renovada!")
                    time.sleep(1)
                    st.rerun()
        ...
        # --- ABAS COMUNS (Perfil System e Perfil Gerencial Clientes) ---
                ...
                for idx, row in df_pendente.iterrows():
                    ...
                    with st.container(border=True):
                        ...
                        with c1:
                            ...
                            st.caption(f"📧 {row['EMAIL']} | 🏢 {str(row.get('EMPRESA', 'N/D')).replace('None', 'N/D')}")
                        with c2:
            ...
            # Filtros de busca e ordenação (MANTIDOS)
            ...
            if user_p['PERFIL'] == 'Perfil System':
                # Busca as empresas direto do histórico para o filtro
                df_h_temp = ler_sql("SELECT DISTINCT EMPRESA FROM historico_loto")
                lista_empr_filt = ["Todas"] + sorted([str(x) for x in df_h_temp['EMPRESA'].unique() if x and str(x).lower() != 'nan'])
                filtro_emp_arq = c_f3.selectbox("🏢 Empresa (Filtro):", lista_empr_filt)
            else:
                filtro_emp_arq = user_p.get('EMPRESA', '')
                c_f3.info(f"🏢 {filtro_emp_arq}")

            # Botão para voltar (Windows Explorer Style)
            if st.session_state.pasta_atual != "":
                if st.button(f"⬅️ Voltar para a Raiz", use_container_width=True):
                    st.session_state.pasta_atual = ""
                    st.rerun()

            try:
                # 1. Lista o conteúdo do caminho atual
                res_storage = conn.client.storage.from_("evidencias").list(path=st.session_state.pasta_atual)
                
                if not res_storage:
                    st.info("Esta pasta está vazia.")
                else:
                    # 2. LÓGICA DE PRIVACIDADE (ADMIN MASTER VÊ TUDO)
                    arquivos_filtrados = []
                    
                    if user_p['PERFIL'] == 'Perfil System' and filtro_emp_arq == "Todas":
                        # Master sem filtro: vê tudo que o Supabase retornou
                        arquivos_filtrados = res_storage
                    else:
                        # 1. Define qual empresa estamos filtrando
                        empresa_alvo = filtro_emp_arq if user_p['PERFIL'] == 'Perfil System' else user_p['EMPRESA']
                        
                        # 2. Busca no histórico quais OS e Fotos pertencem a essa empresa
                        df_vinc = ler_sql(f"SELECT OS, OBS_GERAL FROM historico_loto WHERE EMPRESA = '{empresa_alvo}'")
                        
                        oss_permitidas = []
                        fotos_permitidas = []
                        
                        if not df_vinc.empty:
                            # Lista de OSs da empresa (para filtrar as pastas)
                            oss_permitidas = df_vinc['OS'].astype(str).unique().tolist()
                            
                            # Lista de caminhos de fotos (para filtrar os arquivos)
                            for obs in df_vinc['OBS_GERAL'].dropna():
                                if "Fotos: " in str(obs):
                                    nomes = str(obs).split("Fotos: ")[-1].split(", ")
                                    fotos_permitidas.extend([n.strip() for n in nomes])
                        
                        # 3. Aplica o filtro na lista do Storage
                        arquivos_filtrados = []
                        for item in res_storage:
                            nome = item['name']
                            is_pasta = item.get('id') is None
                            
                            if is_pasta:
                                # Só mostra a pasta se o nome dela (OS) pertencer à empresa
                                if nome in oss_permitidas:
                                    arquivos_filtrados.append(item)
                            else:
                                # Só mostra o arquivo se o caminho dele estiver no histórico da empresa
                                caminho_full = f"{st.session_state.pasta_atual}/{nome}" if st.session_state.pasta_atual else nome
                                if caminho_full in fotos_permitidas:
                                    arquivos_filtrados.append(item)

                    # 3. GRID DE EXIBIÇÃO (FIX: contador_real definido aqui)
                    cols = st.columns(4)
                    contador_real = 0 # <--- INICIALIZAÇÃO QUE FALTAVA
                    ...
                    for item in arquivos_filtrados:
                        ...
                        if busca_arq and busca_arq.lower() not in nome.lower():
                            continue

                        with cols[contador_real % 4]:
                            with st.container(border=True):
                                if is_pasta:
                                    st.write(f"📁 **{nome}**")
                                    if st.button("Abrir", key=f"open_{nome}_{contador_real}"):
                                        st.session_state.pasta_atual = nome
                                        st.rerun()
                                else:
                                    caminho = f"{st.session_state.pasta_atual}/{nome}" if st.session_state.pasta_atual else nome
                                    url = conn.client.storage.from_("evidencias").get_public_url(caminho)
                                    ...
                                    if c_del.button("🗑️", key=f"del_{nome}_{contador_real}"):
                                        conn.client.storage.from_("evidencias").remove([caminho])
                                        st.rerun()
                        contador_real += 1 # Incrementa apenas para itens mostrados
            except Exception as e:
