...
def ler_sql(query, params=()):
    ...
    try:
        ...
        # 3. Limpa os nomes das colunas (Remove espaços e deixa em MAIÚSCULO)
        df_gs.columns = [str(c).strip().upper() for c in df_gs.columns]

        # 4. Lógica de Filtro (Simula o WHERE do SQL)
        if "WHERE" in query_up and params:
            try:
                # Extrai o nome da coluna após o WHERE (ex: EMAIL ou RESP)
                coluna_filtro = query_up.split("WHERE")[1].split("=")[0].strip().upper()
                valor_busca = str(params[0]).strip().lower()
                
                # Filtra o DataFrame comparando tudo em minúsculo e sem espaços
                df_gs = df_gs[df_gs[coluna_filtro].astype(str).str.strip().str.lower() == valor_busca]
            except Exception as e:
                st.error(f"Erro no filtro da coluna {coluna_filtro}: {e}")
                return pd.DataFrame()
