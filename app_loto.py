...
if st.session_state.pagina_ativa == "🏠 Início":
    ...
    # --- 3. MÉTRICAS CLICÁVEIS (HYPERLINKS) ---
    st.write("##")
    c1, c2, c3, c4 = st.columns(4)

    with c1:
        st.markdown('<div class="metric-card-btn">', unsafe_allow_html=True)
        # Usei :blue[valor] para colorir o número
        if st.button(f"Total\n\n:blue[{total_finalizados + total_ativos + total_cancelados}]", key="btn_h_total"):
    ...
    with c2:
        st.markdown('<div class="metric-card-btn">', unsafe_allow_html=True)
        if st.button(f"Em Execução\n\n:orange[{total_ativos}]", key="btn_h_ativos"):
    ...
    with c3:
        st.markdown('<div class="metric-card-btn">', unsafe_allow_html=True)
        if st.button(f"Concluídas\n\n:green[{total_finalizados}]", key="btn_h_conc"):
    ...
    with c4:
        st.markdown('<div class="metric-card-btn">', unsafe_allow_html=True)
        if st.button(f"Canceladas\n\n:red[{total_cancelados}]", key="btn_h_canc"):
