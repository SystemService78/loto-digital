...
# --- DEBUG DE SEGREDOS (RAIO-X) ---
if st.sidebar.checkbox("🔍 Debug: Ver chaves do servidor"):
    st.sidebar.write("Gavetas detectadas no Secrets:", list(st.secrets.keys()))
    if "connections" in st.secrets:
        st.sidebar.write("Dentro de connections:", list(st.secrets["connections"].keys()))
# --- DEBUG DE SEGREDOS (RAIO-X) ---
...
# --- 1. CONEXÃO ---
try:
    # 1. Pegamos os dados da seção 'google_sheets' do seu Secrets
    # Usamos .to_dict().copy() para ter liberdade total de mexer nos dados
    gs_info = st.secrets["google_sheets"].to_dict().copy()
    
    # 2. Corrigimos a chave privada (converte \\n em quebra de linha real)
    if "private_key" in gs_info:
        gs_info["private_key"] = gs_info["private_key"].replace("\\n", "\n")
    
    # 3. Extraímos a URL para uma variável e a removemos do dicionário
    url_final = gs_info.pop("spreadsheet", None)

    # 4. AQUI ESTÁ A SOLUÇÃO DO LOOP: 
    # O Google envia um campo chamado 'type' (service_account).
    # O Streamlit usa um parâmetro chamado 'type' (GSheetsConnection).
    # Se deixarmos os dois, dá erro. Então, APAGAMOS o do Google antes de conectar.
    if "type" in gs_info:
        del gs_info["type"]
    
    # 5. Agora conectamos com segurança. 
    # Passamos o tipo oficial e espalhamos o restante dos dados do Google (**gs_info)
    conn = st.connection("loto_db", type=GSheetsConnection, spreadsheet=url_final, **gs_info)
...
def ler_sql(query, params=()):
    ...
    if not tabela_alvo: return pd.DataFrame()

    try:
        # Lê a aba inteira usando a conexão 'loto_db' que criamos
        df_gs = conn.read(worksheet=tabela_alvo, ttl=0)
        
        if df_gs is None or df_gs.empty:
            # Se for equipamentos, retorna estrutura vazia para não quebrar o site
            if tabela_alvo == 'equipamentos':
                return pd.DataFrame(columns=['SETOR', 'EQUIPAMENTO', 'LINK', 'LINK_ARQUIVO1', 'LINK_ARQUIVO2'])
            return pd.DataFrame()

        # LIMPEZA TOTAL: Colunas em Maiúsculo e sem espaços
        df_gs.columns = [str(c).strip().upper() for c in df_gs.columns]
        
        # Filtro manual (Substitui o WHERE do SQL)
        if "EMAIL" in query_up and params:
            valor_busca = str(params[0]).strip().lower()
            # Filtra garantindo que ambos os lados sejam strings limpas e minúsculas
            df_gs = df_gs[df_gs['EMAIL'].astype(str).str.strip().str.lower() == valor_busca]
            
        return df_gs
    except Exception as e:
        # Mostra o erro real na tela para podermos debugar
        st.sidebar.error(f"Erro de leitura ({tabela_alvo}): {e}")
        return pd.DataFrame()
...
def salvar_dataframe_no_sql(df_novo, tabela):
    try:
        ...
        st.error(f"Erro ao salvar na aba {tabela}: {e}")

#-----------------------------------------------------------------------
# --- 3. CARGA INICIAL DE DADOS (AGORA A FUNÇÃO ler_sql JÁ EXISTE ACIMA) ---
try:
