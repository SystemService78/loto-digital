...
# --- 7. HISTÓRICO E DETALHAMENTO ---
    ...
    if not df_hist.empty:
        ...
        def renderizar_card_na_lista(indice, dados_linha, aba):
            ...
            # --- CÁLCULO DE DURAÇÃO PARA FINALIZADOS ---
            duracao_txt = "---"
            try:
                h1 = datetime.datetime.strptime(str(dados_linha['HORA_INICIO']), "%H:%M:%S")
                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            ...
                            tempo_formatado = "Erro no cálculo"
                    ...
                    if selecao:
                        ...
                        with st.container(border=True):
                            ...
                            if novo_perfil == "Perfil Operação":
                                lista_areas_edit = ["Manutenção Mecânica", "Manutenção Elétrica", "Operação", "SESMT"]
