...
st.markdown("""
    ...
    /* Estilo para o botão "Ver Detalhes" dentro do card - CENTRALIZADO */
    .stButton > button {
    ...
    with c_l2:
        ...
        with st.container(border=True):
            ...
            with st.expander("❓ Esqueci minha senha"):
                ...
                # Campos independentes para a recuperação
                f_nome = st.text_input("Seu Nome / Empresa", key="f_nome")
                f_email = st.text_input("E-mail para Recuperação", key="f_email", placeholder="seu@email.com") # campo adicionado conforme solicitação do usuário de teste
                f_obs = st.text_area("Observações (Opcional)", key="f_obs")
                
                if st.button("Enviar Solicitação", use_container_width=True):
                    # Agora validamos o e-mail local (f_email) e o nome
                    if f_email and f_nome:
                        fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
                        agora_sol = datetime.datetime.now(fuso_br).strftime("%d/%m/%Y %H:%M")
                        
                        # --- GRAVAÇÃO USANDO O E-MAIL DEDICADO ---
                        query = """
                            INSERT INTO solicitacoes (DATA, EMAIL, NOME_EMPRESA, OBSERVACAO, STATUS) 
                            VALUES (?, ?, ?, ?, ?)
                        """
                        # Passamos f_email no lugar de email_in
                        executar_sql(query, (agora_sol, f_email, f_nome, f_obs, "Pendente"))
                        
                        st.success("✅ Solicitação enviada! O administrador analisará seu pedido.")
                    else:
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        st.markdown(f"""
            <p style='margin-top: -20px; color: #64748B; font-size: 1.1rem;'>
                <b>Acesso:</b> {user_p['PERFIL']} | <b>Área:</b> {user_p['AREA']} | <b>Gestor:</b> {user_p['NOME']}
            </p>
