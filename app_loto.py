...
def ler_sql(query_ou_tabela, params=None):
    # Identifica a tabela independente de como foi escrita
    ...
    try:
        query = conn.table(tabela_alvo).select("*")
        
        if params:
            # Filtra sempre usando a coluna em minúsculo (como criamos no SQL Editor)
            if "email" in query_ou_tabela.lower():
                res = query.eq("email", params[0]).execute()
            elif "resp" in query_ou_tabela.lower():
                res = query.eq("resp", params[0]).execute()
            else: res = query.execute()
        else:
            res = query.execute()
        ...
        if not df_res.empty:
            # Converte todos os nomes de colunas que vieram do banco para MAIÚSCULO
            # Assim, seu código antigo que procura por ['SETOR'] continuará funcionando!
...
def executar_sql(query, params=()):
    ...
    try:
        if "DELETE FROM ATIVOS_LOTO" in query_up:
            ...
            # params: EMAIL, SENHA, NOME, AREA, PERFIL, ATIVO
