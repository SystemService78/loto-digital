...
def ler_sql(query_ou_tabela, params=None):
    ...
    try:
        ...
        if not df_res.empty:
            # --- DESCOMPACTAÇÃO AUTOMÁTICA DO HISTÓRICO ---
            # Se a tabela for histórico e tiver a gaveta 'dados_etapas'
            if 'dados_etapas' in df_res.columns:
                # Normaliza o JSON para colunas separadas
                df_json = pd.json_normalize(df_res['dados_etapas'])
                # Junta com o DataFrame original e apaga a gaveta fechada
                df_res = pd.concat([df_res.reset_index(drop=True), df_json.reset_index(drop=True)], axis=1)
                if 'dados_etapas' in df_res.columns:
                    df_res = df_res.drop(columns=['dados_etapas'])
        ...
        print(f"Erro de leitura Supabase: {e}")
        return pd.DataFrame()

def salvar_dataframe_no_sql(df, tabela):
    """Empacota colunas extras em JSONB para o Histórico"""
    try:
        df_save = df.copy()
        df_save.columns = [c.lower() for c in df_save.columns]
        
        if tabela.lower() == "historico_loto":
            # Colunas que são fixas no banco
            fixas = ['data', 'hora_inicio', 'hora_fim', 'os', 'resp', 'setor', 'equip', 'tipo', 'status', 'obs_geral']
            # Tudo o que NÃO for fixo vai para dentro da gaveta JSONB
            colunas_checklist = [c for c in df_save.columns if c not in fixas and c != 'id']
            
            # Cria a coluna de dados_etapas e remove as originais
            df_save['dados_etapas'] = df_save[colunas_checklist].to_dict(orient='records')[0]
            df_save = df_save.drop(columns=colunas_checklist)

        dados = df_save.to_dict(orient='records')
...
def executar_sql(query, params=()):
    """Tradutor Mestre: Mapeia as 1.500 linhas de código para o Supabase"""
    ...
    try:
        ...
        # --- CASO 1: RECUPERAÇÃO DE SENHA (SOLICITAÇÕES) ---
        if "INSERT INTO SOLICITACOES" in query_up:
            dados = {
                "data": params[0],
                "email": params[1],
                "nome_empresa": params[2],
                "observacao": params[3],
                "status": params[4]
            }
            conn.table("solicitacoes").insert(dados).execute()

        # --- CASO 2: NOVO EQUIPAMENTO ---
        elif "INSERT INTO EQUIPAMENTOS" in query_up:
            # Mapeia exatamente para as colunas que você tem no Supabase
            dados = {
                "setor": params[0],
                "equipamento": params[1],
                "link": params[2],
                "link_arquivo1": params[3],
                "link_arquivo2": params[4]
            }
            conn.table("equipamentos").insert(dados).execute()

        # --- CASO 3: NOVO USUÁRIO ---
        elif "INSERT INTO USUARIOS" in query_up:
            dados = {
                "email": params[0], "senha": params[1], "nome": params[2], 
                "area": params[3], "perfil": params[4], "ativo": params[5]
            }
            conn.table("usuarios").insert(dados).execute()

        # --- CASO 4: EXCLUSÃO (EQUIPAMENTO OU USUÁRIO) ---
        ...
        # --- CASO 5: MARCAR COMO RESOLVIDO (SOLICITAÇÕES) ---
