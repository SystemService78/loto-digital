                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
                            agora_br = datetime.datetime.now(fuso_br).replace(tzinfo=None)
                            h_ini_raw = str(info_loto.get('HORA_INICIO', '')) # Na Visão Geral use row.get
                            
                            if h_ini_raw and h_ini_raw != 'None':
                                dt_ini = None
                                # Tenta converter nos 3 formatos possíveis que já usamos
                                ...
                                if dt_ini:
                                    # Se o início parece ser no futuro, ele é de ontem
                                    if dt_ini > agora_br and len(h_ini_raw) <= 8:
                                        dt_ini -= datetime.timedelta(days=1)
                                    
                                    delta = agora_br - dt_ini
                                    total_sec = abs(int(delta.total_seconds()))
                                    ...
                                    if dias > 0:
                                        tempo_formatado = f"{int(dias)}d {int(horas):02d}:{int(minutos):02d}:{int(segundos):02d}"
                                    else:
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        # --- ABAS COMUNS (Perfil System e Perfil Gerencial Clientes) ---
            ...
            try:
                # 1. Busca RECURSIVA (Obrigatório para ver dentro de pastas)
                ...
                if user_p['PERFIL'] != 'Perfil System' or filtro_emp_arq != "Todas":
                    ...
                    # Filtra a lista bruta comparando o caminho completo do arquivo
                    arquivos_filtrados = [a for a in res_storage if a['name'] in fotos_permitidas]
                else:
                    # Admin System vê tudo que não for o marcador de pasta vazio
                    arquivos_filtrados = [a for a in res_storage if not a['name'].endswith('/')]

                # 2. Ordenação por Data de Upload
                if ordem_arq == "Mais recentes":
                    arquivos_filtrados.sort(key=lambda x: x.get('created_at', ''), reverse=True)
                elif ordem_arq == "Mais antigos":
                    arquivos_filtrados.sort(key=lambda x: x.get('created_at', ''))
                else:
