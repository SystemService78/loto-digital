                ...
                with col_f1:
                    if all(status_etapas):
                        if st.button("🏁 Finalizar e Registrar", type="primary", use_container_width=True, key="btn_fin_sql"):
                            ...
                            # 1. Tratamento da Foto (Uso do conn.client para acessar Storage)
                            if foto:
                                try:
                                    nome_arquivo = f"{os_n}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                                    # O 'conn.client' é a chave para acessar o Storage diretamente
                            ...
                            # 2. Prepara pacote completo de FINALIZAÇÃO (Sua lógica original)
                            dados_h = {
                                "DATA": [datetime.datetime.now().strftime("%d/%m/%Y")],
                                "HORA_FIM": [datetime.datetime.now().strftime("%H:%M:%S")],
                            ...
                            # 3. Gravação e Limpeza (Golpe Final no erro de pendência)
                            if salvar_dataframe_no_sql(pd.DataFrame(dados_h), "historico_loto"):
                                # Deletamos direto via cliente para não haver erro de 'tuple index'
                ...
                with col_f2:
                    if st.button("❌ Cancelar Registro", use_container_width=True, key="btn_canc_sql"):
                        ...
                        if salvar_dataframe_no_sql(pd.DataFrame(dados_c), "historico_loto"):
                            # Deletamos direto via cliente aqui também
                            conn.client.table("ativos_loto").delete().eq("resp", resp_n).execute()
