...
st.markdown("""
    ...
    /* Ajuste para o Segmented Control (Novas Abas) */
    ...
    div[data-testid="stSegmentedControl"] button[aria-checked="true"] {
        background-color: #FF6B00 !important; /* Cor Laranja para o selecionado */
                    ...
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            ...
                            if h_ini_str:
                                ...
                                # Se o início ficou maior que agora (conflito de fuso), recua 24h
                                if dt_ini > agora:
                                    dt_ini -= datetime.timedelta(days=1)
                                
                                # Cálculo da distância absoluta em segundos
                                diff_sec = int((agora - dt_ini).total_seconds())
                                
                                horas = diff_sec // 3600
                                minutos = (diff_sec % 3600) // 60
                                tempo_formatado = f"{int(horas)}h {int(minutos)}min"
                        except:
                            tempo_formatado = "Calculando..."
