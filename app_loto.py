...
def ler_sql(query_ou_tabela, params=None):
    ...
    try:
        if params:
            # Se a busca for por email (Login)
            if "email" in query_ou_tabela.lower():
                res = conn.table(tabela_alvo).select("*").eq("email", params[0]).execute()
            # Se a busca for por responsável (Ativos)
            elif "resp" in query_ou_tabela.lower():
...
def salvar_dataframe_no_sql(df, tabela):
    """Converte o DataFrame do Pandas para o formato que o Supabase entende e salva"""
    try:
...
def executar_sql(query, params=()):
    """Lida com comandos de INSERT, UPDATE e DELETE"""
    ...
    try:
        if "DELETE FROM ATIVOS_LOTO" in query_up:
            ...
            # Ordem do seu cadastro: EMAIL, SENHA, NOME, AREA, PERFIL, ATIVO
            dados = {"email": params[0], "senha": params[1], "nome": params[2], 
                     "area": params[3], "perfil": params[4], "ativo": params[5]}
...
def salvar_ativo_sql(resp, dados_dict):
    # O comando UPSERT insere se não existir ou atualiza se já existir
