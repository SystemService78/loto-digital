                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
                            agora_br = datetime.datetime.now(fuso_br).replace(tzinfo=None)
                            h_ini_str = info_loto.get('HORA_INICIO', '') # No histórico use row.get
                            
                            if h_ini_str:
                                if len(h_ini_str) <= 8: # Apenas HH:MM:SS
                                    hoje_str = agora_br.strftime("%d/%m/%Y")
                                    dt_ini = datetime.datetime.strptime(f"{hoje_str} {h_ini_str}", "%d/%m/%Y %H:%M:%S")
                                else: # DD/MM/YYYY HH:MM:SS ou DD/MM/YYYY HH:MM
                                    try:
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        # --- ABAS COMUNS (Perfil System e Perfil Gerencial Clientes) ---
            ...
            try:
                # 1. Busca recursiva (lista todos os arquivos em todas as subpastas)
                lista_bruta = conn.client.storage.from_("evidencias").list(options={"recursive": True})
                ...
                if not lista_bruta:
                    ...
                    # 2. Ordenação Segura
                    arquivos_filtrados = [a for a in lista_bruta if busca_arq.lower() in a['name'].lower() and not a['name'].endswith('/')]

                    if ordem_arq == "Mais recentes":
                        # O .get garante que se o campo for nulo, o sistema não trave
                        arquivos_filtrados.sort(key=lambda x: x.get('created_at') or "", reverse=True)
                    elif ordem_arq == "Mais antigos":
                        arquivos_filtrados.sort(key=lambda x: x.get('created_at') or "")
                    else:
                        arquivos_filtrados.sort(key=lambda x: x.get('name') or "")
