...
# --- 10. PÁGINA DE SUPORTE ---
    ...
    with col2:
        ...
        st.link_button("Abrir WhatsApp Suporte", "https://wa.me/seunumeroaqui", use_container_width=True)
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        if escolha_aba_admin == "➕ Novo Usuário":
            ...
            with st.container(border=True):
                ...
                if st.button("🚀 Confirmar Renovação e Liberar Acesso", type="primary", use_container_width=True):
                    # Grava na tabela de licenças (Usa Upsert: se já existe, atualiza. Se não, cria.)
                    conn.table("licencas").upsert({"empresa": emp_sel_lic, "data_vencimento": str(data_fim)}).execute()
                    
                    # Reativa todos os usuários dessa empresa imediatamente
                    executar_sql("UPDATE usuarios SET ATIVO = 'Sim' WHERE EMPRESA = ?", (emp_sel_lic,))
                    
                    st.success(f"✅ Licença renovada e usuários da {emp_sel_lic} reativados!")
                    time.sleep(1)
                    st.rerun()
