...
# --- 4. SIDEBAR ---
with st.sidebar:
    if st.button("🔄 Sincronizar Dados", use_container_width=True, help="Atualiza informações do Banco de Dados"):
                ...
                for idx, row in df_pendente.iterrows():
                    ...
                    with st.container(border=True):
                        ...
                        with c3:
                            ...
                            if st.button("✅ Resolver", key=f"btn_sol_{row['ID']}", use_container_width=True):
                                # Importante: Usamos o ID específico da linha
                                executar_sql("UPDATE solicitacoes SET STATUS = 'Resolvido' WHERE ID = ?", (row['ID'],))
                                st.toast(f"Solicitação de {row['NOME_EMPRESA']} resolvida!")
                                time.sleep(0.5)
                                st.rerun()
