...
def ler_sql(query_ou_tabela, params=None):
    ...
    try:
        ...
        # --- FILTRO GENÉRICO ---
        query_up = query_ou_tabela.upper()
        if "WHERE" in query_up:
            # Divide a query para pegar o que vem depois do WHERE
            # Ex: STATUS = 'Pendente' ou EMAIL = ?
            ...
            if "=" in parte_filtro:
                ...
                # Se o valor for o marcador '?', usamos o params[0]
                if valor == "?" and params:
                    q = q.eq(coluna, params[0])
                else:
                    q = q.eq(coluna, valor)
        ...
        if not df_res.empty:
            # Descompacta JSONB se for Histórico
            ...
            # Padroniza para MAIÚSCULO para o resto do código
...
def salvar_dataframe_no_sql(df, tabela):
    """Empacota colunas extras em JSONB de forma segura para o Supabase"""
    try:
        df_save = df.copy()
        # Converte nomes das colunas para minúsculo para o banco
        df_save.columns = [str(c).lower().strip() for c in df_save.columns]
        
        if tabela.lower() == "historico_loto":
            # 1. Definimos as colunas que têm gaveta própria no Supabase
            colunas_fixas = ['data', 'hora_inicio', 'hora_fim', 'os', 'resp', 'setor', 'equip', 'tipo', 'status', 'obs_geral']
            
            # 2. Tudo que não for fixo, vai para o 'dados_etapas' (Checklist)
            cols_checklist = [c for c in df_save.columns if c not in colunas_fixas and c != 'id']
            
            # 3. Transformamos as colunas de checklist em um dicionário (JSON)
            # Pegamos apenas a primeira linha do dicionário gerado
            lista_dict = df_save[cols_checklist].to_dict(orient='records')
            df_save['dados_etapas'] = [lista_dict[0]] if lista_dict else [{}]
            
            # 4. Removemos as colunas originais do checklist para não dar erro de "coluna inexistente"
        ...
        st.error(f"Erro ao salvar no Supabase ({tabela}): {e}")
        return False
        ...
        # --- CASO C: ATUALIZAÇÃO (UPDATE) ---
            ...
            if tabela_alvo == "solicitacoes":
                ...
                if "SET SENHA" in query_up:
                    ...
                    d_u = {"nome": params[0], "ativo": params[1], "area": params[2], "perfil": params[3]}
                    conn.table("usuarios").update(d_u).eq("email", params[4]).execute()
            elif tabela_alvo == "equipamentos":
...
# --- MÉTRICAS REAIS VIA SQL (FILTRADAS POR PERFIL) ---
    ...
    empresa_logada = user_atual.get('EMPRESA', '')
    ...
    try:
        if perfil_logado == 'Admin':
            ...
            # Admin Cliente vê tudo da EMPRESA dele
            df_h_count = ler_sql("SELECT STATUS FROM historico_loto WHERE EMPRESA = ?", (empresa_logada,))
            df_a_count = ler_sql("SELECT RESP FROM ativos_loto WHERE EMPRESA = ?", (empresa_logada,))
        else:
            # Operador vê apenas o DELE
                    ...
                    if all(status_etapas):
                        if st.button("🏁 Finalizar e Registrar", type="primary", use_container_width=True, key="btn_finalizar_unico"):
                            ...
                            if salvar_dataframe_no_sql(pd.DataFrame(dados_h), "historico_loto"):
                                # Deleta o registro pendente para ele sumir da tela inicial
                                conn.client.table("ativos_loto").delete().eq("resp", resp_n).execute()
    ...
    if not df_hist.empty:
        ...
        if st.session_state.id_os_detalhe is not None:
            ...
            if not df_filtrado.empty:
                ...
                with c_down_info:
                    st.markdown(f"📊 **Relatório Gerado:** {len(df_filtrado)} registros encontrados com os filtros atuais.")
                    st.caption("O download incluirá todos os dados filtrados, ignorando a paginação da tela.")

                with c_down_btn:
                    # Preparando o arquivo Excel em memória
                    buffer = io.BytesIO()
...
# Perfil Adm
    ...
    # Agora Admin E Admin Cliente entram aqui
    if user_p['PERFIL'] in ['Admin', 'Admin Cliente']:
        st.title(f"⚙️ Gestão Administrativa - {user_p.get('EMPRESA', 'Geral')}")
        ...
        # Lógica de Abas Diferenciadas
        if user_p['PERFIL'] == 'Admin':
            # Admin Geral vê todas as 5 abas
            tab_cadastro, tab_gerenciamento, tab_equipamentos, tab_senha, tab_arquivos = st.tabs([
                "➕ Novo Usuário", "📝 Editar Usuário", "🛠️ Equipamentos", label_solicitacoes, "📁 Arquivos/Fotos"
            ])
        else:
            # Admin Cliente vê apenas Arquivos e Solicitações (ou o que você preferir)
            # Criamos apenas as abas permitidas para ele
            tab_arquivos, tab_senha = st.tabs(["📁 Arquivos/Fotos", label_solicitacoes])
            # Criamos variáveis vazias para as abas que ele não tem, para não dar erro no código abaixo
            tab_cadastro = tab_gerenciamento = tab_equipamentos = st.container()
        ...
        with tab_cadastro:
            if user_p['PERFIL'] == 'Admin': # Proteção extra
                ...
                def finalizar_cadastro_callback():
                    ...
                    #empresa = st.text_input("Empresa do Colaborador", placeholder="Ex: Cliente XPTO")
                ...
                with st.container(border=True):
                    ...
                    st.text_input("Empresa", key="reg_empresa", placeholder="Ex: System Service") # WIDGET AQUI
        ...
        with tab_arquivos:
            ...
            try:
                ...
                if not lista_bruta:
                    st.info("Nenhum arquivo encontrado no bucket 'evidencias'.")
                else:
