...
st.markdown("""
    ...
    .hist-card {
        ...
        padding: 15px !important;
        border-radius: 12px !important;
        border-left: 6px solid #FF6B00 !important; /* Faixa laranja lateral */
...
with st.sidebar:
    ...
    for opcao in OPCOES_MENU:
        # Define o estilo: Laranja se for a página ativa, cinza se não for
        estilo = "primary" if st.session_state.pagina_ativa == opcao else "secondary"
        if st.button(opcao, key=f"menu_{opcao}", use_container_width=True, type=estilo):
