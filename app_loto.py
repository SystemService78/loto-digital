    ...
    if maquina_sel:
        ...
        if os_n:
            ...
            with st.container(border=True):
                ...
                with col_f1:
                    # BOTÃO FINALIZAR (Apenas se 10/10 etapas concluídas)
                    if all(status_etapas):
                        if st.button("🏁 Finalizar e Registrar", type="primary", use_container_width=True, key="btn_fin_sql"):
                            nome_foto_db = None
                            
                            # 1. Tratamento da Foto (Supabase Storage)
                            if foto:
                                try:
                                    nome_arquivo = f"{os_n}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                                    conn.storage.from_("evidencias").upload(
                                        path=nome_arquivo,
                                        file=foto.getvalue(),
                                    ...
                                    st.warning(f"Atenção: Erro ao subir foto, mas o registro continuará. ({e})")


                            # 2. Prepara pacote completo de FINALIZAÇÃO
                            dados_h = {
                                "DATA": [datetime.datetime.now().strftime("%d/%m/%Y")],
                                "HORA_FIM": [datetime.datetime.now().strftime("%H:%M:%S")],
                            ...
                            # Inclui timestamps e notas das etapas
                            ...
                            # 3. Salva no Histórico e remove Ativo
                            if salvar_dataframe_no_sql(pd.DataFrame(dados_h), "historico_loto"):
                                executar_sql(f"DELETE FROM ativos_loto WHERE RESP = '{resp_n}'")
                                ...
                                st.balloons()
                                st.toast("Bloqueio registrado com sucesso!")
                                time.sleep(1)
                                navegar_para("📋 Minhas Execuções")
                    else:
                        st.info("💡 Conclua o checklist para habilitar a finalização.")

                with col_f2:
                    # BOTÃO CANCELAR (Restaura toda a sua lógica original de campos)
                    if st.button("❌ Cancelar Registro", use_container_width=True, key="btn_canc_sql"):
                        # 1. Prepara pacote completo de CANCELAMENTO (Sem perder nenhum dado)
                        ...
                        # 2. Grava no Histórico e remove dos Ativos
...
# Perfil Adm
    ...
    if st.session_state.usuario['PERFIL'] == 'Admin':
        ...
        tab_cadastro, tab_gerenciamento, tab_equipamentos, tab_senha = st.tabs([
            "➕ Novo Usuário", "📝 Editar Usuário", "🛠️ Equipamentos", label_solicitacoes
        ])
