...
##trecho removido daqui
    ...
    if maquina_sel:
        ...
        with st.container(border=True):
            ...
            # --- VALIDAÇÃO DE CONFLITO A NÍVEL EMPRESA (ENGENHARIA DE DADOS) ---
            # 1. Buscamos todos os ativos para checar se a máquina já está travada por alguém da empresa
            ...
            for reg_db in lista_ativos:
                # --- AJUSTE DE ENGENHARIA: Tenta buscar a chave em minúsculo ou maiúsculo ---
                d_json = reg_db.get('dados_json') or reg_db.get('DADOS_JSON')
                
                # Só prossegue se o dado JSON existir no registro
                if d_json:
                    # Usamos .get() também dentro do JSON para evitar novos KeyErrors
                    equip_db = d_json.get('EQUIP')
                    tipo_db = d_json.get('TIPO')
                    empresa_db = str(d_json.get('EMPRESA', '')).strip()
                    empresa_atual = str(st.session_state.usuario.get('EMPRESA', '')).strip()

                    # Verificamos se: Mesma Máquina E Mesmo Tipo E Mesma Empresa
                    if (str(equip_db) == str(maquina_sel) and 
                        str(tipo_db) == str(tipo) and 
                        empresa_db == empresa_atual):
                        conflito_v5 = reg_db
                        # Guardamos o JSON padronizado para uso posterior nos botões
                        conflito_v5['DADOS_JSON_FIX'] = d_json 
                        break

            # Se houver conflito e não estivermos já editando (vindo da Home)
            if conflito_v5 and st.session_state.editando_agora == False:
                # Usamos a nossa chave fixada
                dados_retomada = conflito_v5['DADOS_JSON_FIX']
                
                # Pega o ID (resp) para saber quem é o dono
                id_resp = conflito_v5.get('resp', '')
                dono_bloqueio = id_resp.split('|')[0] if '|' in id_resp else id_resp

                # --- CENÁRIO A: O BLOQUEIO PERTENCE AO USUÁRIO LOGADO (RESTAURAÇÃO COMPLETA) ---
                if dono_bloqueio == user_nome:
                    st.warning(f"⚠️ {user_nome}, você já possui um bloqueio de **{tipo}** em aberto para esta máquina.")
                    
                    # Alerta de Segurança: Máquina fora de operação
                    c_conf = sum(1 for i in range(7) if dados_retomada.get("TIMESTAMPS", {}).get(f"t_{itens_loto[i][1]}") or dados_retomada.get("NA_ETAPAS", {}).get(f"na_{itens_loto[i][1]}"))
                    if c_conf >= 7:
                        st.error(f"🚨 **ALERTA DE SEGURANÇA:** A máquina **{maquina_sel}** encontra-se **FORA DE OPERAÇÃO**.")

                    c_ret1, c_ret2 = st.columns(2)
                    
                    if c_ret1.button("▶️ Continuar Meu Trabalho", use_container_width=True, key="btn_retomar_v5"):
                        # REPOSIÇÃO INTEGRAL DO ESTADO (O seu 'Pulo do Gato')
                    ...
                    if c_ret2.button("➕ Iniciar Novo (Descarta antigo)", use_container_width=True, key="btn_novo_v5"):
                        # REGISTRO DE CANCELAMENTO NO HISTÓRICO (Rastreabilidade)
                        dados_p_hist = {
                        ...
                        # Preenche colunas das etapas para o histórico cancelado
                        for _, n_it in itens_loto:
                            u_n = n_it.upper()
                            dados_p_hist[f"HORA_{u_n}"] = [dados_retomada.get('TIMESTAMPS', {}).get(f"t_{n_it}", "N/A")]
                            dados_p_hist[f"OBS_{u_n}"] = [dados_retomada.get('OBS_ETAPAS', {}).get(n_it, "")]
                            dados_p_hist[f"NA_{u_n}"] = [dados_retomada.get('NA_ETAPAS', {}).get(f"na_{n_it}", False)]

                        salvar_dataframe_no_sql(pd.DataFrame(dados_p_hist), "historico_loto")
                        conn.client.table("ativos_loto").delete().eq("resp", conflito_v5['resp']).execute()
                        
                        # Limpeza cirúrgica da memória
                        ...
                        st.session_state.editando_agora = True
                        st.rerun()
                    st.stop() # Bloqueia o formulário até a decisão ser tomada

                # --- CENÁRIO B: O BLOQUEIO PERTENCE A UM COLEGA (TRAVA DE SEGURANÇA) ---
                else:
                    st.error(f"🚫 **EQUIPAMENTO BLOQUEADO:** O colaborador **{dono_bloqueio}** já possui um registro de **{tipo}** para esta máquina.")
                    st.info(f"Para sua segurança, você não pode intervir até que a OS **{dados_retomada.get('OS')}** seja concluída ou liberada por **{dono_bloqueio}**.")
                    
                    # Alerta se o colega já estiver em fase crítica
                    c_col = sum(1 for i in range(7) if dados_retomada.get("TIMESTAMPS", {}).get(f"t_{itens_loto[i][1]}") or dados_retomada.get("NA_ETAPAS", {}).get(f"na_{itens_loto[i][1]}"))
                    if c_col >= 7:
                        st.warning(f"🚨 Atenção: {dono_bloqueio} já removeu as proteções ou está em execução avançada.")
