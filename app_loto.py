...
# --- 7. HISTÓRICO E DETALHAMENTO ---
    ...
    if not df_hist.empty:
        ...
        if st.session_state.id_os_detalhe is not None:
            ...
            elif user_atual['PERFIL'] == 'Admin Cliente':
            ...
            if st.session_state.get('f_resp', 'Todos') != "Todos" and user_atual['PERFIL'] == 'Admin':
            ...
            if not df_andamento.empty:
                # Aplicar filtro de Empresa/Perfil nas Ativas para a contagem bater
                if user_atual['PERFIL'] == 'Admin':
                    if st.session_state.f_empresa != "Todas":
            ...
            # --- BLOCO DE DOWNLOAD (EXCEL) DINÂMICO ---
            # Identifica qual a aba atual para a contagem e o download
            status_atual = st.session_state.get('aba_desejada', "Todas")
            
            status_atual = st.session_state.get('aba_desejada', "Todas")
            
            if status_atual == "Todas":
                # 1. Preparar as ativas para o Excel (Transformar o JSON em colunas)
                df_ativas_prep = pd.DataFrame()
                if not df_andamento.empty:
                    # Normaliza o JSON para colunas e garante que o Status apareça
                    df_ativas_prep = pd.json_normalize(df_andamento['DADOS_JSON'])
                    df_ativas_prep['STATUS'] = "EM EXECUÇÃO"
                    # Mapeia colunas do JSON para o padrão do Histórico (Ex: EQUIP -> EQUIPAMENTO)
                    if 'EQUIP' in df_ativas_prep.columns:
                        df_ativas_prep['EQUIPAMENTO'] = df_ativas_prep['EQUIP']

                # 2. Une o Histórico filtrado com as Ativas filtradas
                # Isso garante que as 4 do histórico + 11 ativas = 15 no total
                df_para_excel = pd.concat([df_filtrado, df_ativas_prep], axis=0, ignore_index=True)
                qtd_resumo = len(df_para_excel)
                
            elif status_atual == "Ativas":
                qtd_resumo = len(df_andamento)
                # Se baixar só ativas, processa o JSON delas
                if not df_andamento.empty:
                    df_para_excel = pd.json_normalize(df_andamento['DADOS_JSON'])
                    df_para_excel['STATUS'] = "EM EXECUÇÃO"
                else:
                    df_para_excel = pd.DataFrame()

            # Exibição do resumo baseada na aba clicada
            st.write("---")
            c_down_info, c_down_btn = st.columns([3, 1])
            with c_down_info:
                st.markdown(f"📊 **Relatório Gerado ({status_atual}):** {qtd_resumo} registros encontrados.")
            ...
            with c_down_btn:
                if not df_para_excel.empty:
                    ...
                    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                        df_excel_final = df_para_excel.copy()
                        
                        # Cálculo de duração apenas se houver as colunas necessárias
                        if 'HORA_INICIO' in df_excel_final.columns and 'HORA_FIM' in df_excel_final.columns:
                            def calc_dur(row):
                                try:
                                    t1 = datetime.datetime.strptime(str(row['HORA_INICIO']), "%H:%M:%S")
                                    t2 = datetime.datetime.strptime(str(row['HORA_FIM']), "%H:%M:%S")
                                    return str(t2 - t1)
                                except: return "---"
                            df_excel_final['DURACAO_TOTAL'] = df_excel_final.apply(calc_dur, axis=1)
                        
                        df_excel_final.to_excel(writer, index=False, sheet_name='Relatório')
                    
                    st.download_button(
                        label=f"📥 Baixar {status_atual}",
                        data=buffer.getvalue(),
                        file_name=f"LOTO_{status_atual}.xlsx",
            ...
            # 4. SISTEMA DE NAVEGAÇÃO POR STATUS (Substituindo Tabs para suportar o clique da Home)
            opcoes_status = ["Todas", "Ativas", "Concluídas", "Canceladas"]
            
            # Verifica se existe um pedido de aba vindo da Home, se não houver, usa "Todas"
            ...
            elif escolha_status == "Canceladas":
            ...
            # Botão de Nova Execução rápido no rodapé (Fora dos ifs para aparecer sempre)
            st.write("##")
    ...
    if user_p['PERFIL'] in ['Admin', 'Admin Cliente']:
        ...
        with tab_arquivos:
            ...
            try:
                ...
                if user_p['PERFIL'] != 'Admin' or filtro_emp_arq != "Todas":
