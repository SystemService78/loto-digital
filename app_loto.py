...
def carregar_dados():
    ...
    if not df_temp.empty:
        ...
        # FILTRO DE SEGURANÇA: Se não for Admin Geral, só vê os equipamentos da sua empresa
        u = st.session_state.get('usuario')
        if u and u['PERFIL'] != 'Admin':
            emp = str(u.get('EMPRESA', ''))
            if 'EMPRESA' in df_temp.columns:
                df_temp = df_temp[df_temp['EMPRESA'].astype(str) == emp]
        
        return df_temp.fillna("")
    return pd.DataFrame(columns=['SETOR', 'EQUIPAMENTO', 'LINK_RAIZ', 'LINK_MAPA', 'LINK_PROCEDIMENTO', 'EMPRESA'])
...
# --- MÉTRICAS REAIS VIA SQL (FILTRADAS POR PERFIL) ---
    ...
    empresa_logada = str(user_atual.get('EMPRESA', ''))

    try:
        # Buscamos tudo e limpamos o index para evitar o erro de reindex
        ...
        if perfil_logado == 'Admin':
            ...
            # Filtro por Empresa
            df_h_count = df_h_tudo[df_h_tudo['EMPRESA'].astype(str) == empresa_logada] if not df_h_tudo.empty else pd.DataFrame()
            df_a_count = df_a_tudo[df_a_tudo['EMPRESA'].astype(str) == empresa_logada] if not df_a_tudo.empty else pd.DataFrame()
        else:
            # Filtro por Operador
            df_h_count = df_h_tudo[df_h_tudo['RESP'] == user_atual['NOME']] if not df_h_tudo.empty else pd.DataFrame()
            df_a_count = df_a_tudo[df_a_tudo['RESP'] == user_atual['NOME']] if not df_a_tudo.empty else pd.DataFrame()
    ...
    if not df_hist.empty:
        ...
        if not df_hist.empty:
            if user_atual['PERFIL'] == 'Admin Cliente':
                df_hist = df_hist[df_hist['EMPRESA'].astype(str) == str(user_atual['EMPRESA'])]
