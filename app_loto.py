    ...
    with c_l2:
        ...
        with st.container(border=True):
            ...
            with st.expander("❓ Esqueci minha senha"):
                ...
                f_email = st.text_input("E-mail para Recuperação", key="f_email", placeholder="seu@email.com")
                ...
                if st.button("Enviar Solicitação", use_container_width=True):
                    if f_email and f_nome and f_empresa_rec:
                        ...
                        # --- GRAVAÇÃO USANDO NOME E EMPRESA SEPARADOS ---
                        # Note: params[2] é NOME_EMPRESA (Nome) e adicionei a coluna EMPRESA no final
                        query = """
                            INSERT INTO solicitacoes (DATA, EMAIL, NOME_EMPRESA, OBSERVACAO, STATUS, EMPRESA) 
                            VALUES (?, ?, ?, ?, ?, ?)
                        """
                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
                            agora_br = datetime.datetime.now(fuso_br).replace(tzinfo=None)
                            h_ini_str = info_loto.get('HORA_INICIO', '') # No loop da Visão Geral use row.get
                            
                            if h_ini_str:
                                if len(h_ini_str) <= 8: # Formato HH:MM:SS
                                    hoje_str = agora_br.strftime("%d/%m/%Y")
                                    dt_ini = datetime.datetime.strptime(f"{hoje_str} {h_ini_str}", "%d/%m/%Y %H:%M:%S")
                                    
                                    # SE DEU NEGATIVO: Significa que começou ontem. Subtraímos 1 dia da data inicial.
                                    if dt_ini > agora_br:
                                ...
                                # Garante que o tempo nunca seja exibido como negativo na tela
