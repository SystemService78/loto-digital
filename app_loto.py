...
##trecho removido daqui
    ...
    if maquina_sel:
        ...
        with st.container(border=True):
            ...
            # --- ENGENHARIA DE CONFLITOS (PASSO 2 COMPLETO) ---
            ...
            for reg_db in lista_ativos:
                ...
                if d_json:
                    ...
                    # Nome do responsável no ID do banco
                    dono_id = reg_db.get('resp', '').split('|')[0]

                    # CONDIÇÃO: Mesma Máquina + Mesmo Tipo + Mesma Empresa + DONO DIFERENTE
            ...
            if st.session_state.editando_agora == False:
                ...
                # CENÁRIO A: O conflito é do próprio Hugo (Hugo vê seus botões de Retomar/Cancelar)
                if conflito_direto:
                    st.warning(f"⚠️ {user_nome}, você já possui um bloqueio de **{tipo}** em aberto para esta máquina.")
                    
                    # Alerta de Segurança: Máquina fora de operação (7/10)
                    prog_c = sum(1 for i in range(7) if conflito_direto.get("TIMESTAMPS", {}).get(f"t_{itens_loto[i][1]}") or conflito_direto.get("NA_ETAPAS", {}).get(f"na_{itens_loto[i][1]}"))
                    if prog_c >= 7:
                    ...
                    if c_ret1.button("▶️ Continuar Meu Trabalho", use_container_width=True, key="btn_retomar_v_final"):
                        # REPOSIÇÃO INTEGRAL DO ESTADO (Pulo do Gato)
                    ...
                    if c_ret2.button("➕ Iniciar Novo (Descarta antigo)", use_container_width=True, key="btn_novo_v_final"):
                        ...
                        # Deleta especificamente o registro do Hugo para aquela máquina/tipo
                        conn.client.table("ativos_loto").delete().eq("resp", f"{user_nome}|{maquina_sel}|{tipo}").execute()
                ...
                # CENÁRIO B: O conflito é de um COLEGA (Hugo é impedido de abrir o MESMO tipo)
                    ...
                    # Alerta 7/10 do colega
                    c_col = sum(1 for i in range(7) if d_col.get("TIMESTAMPS", {}).get(f"t_{itens_loto[i][1]}") or d_col.get("NA_ETAPAS", {}).get(f"na_{itens_loto[i][1]}"))
                    if c_col >= 7:
                        st.warning(f"🚨 Atenção: {dono_bloqueio} já encontra-se em estágio crítico de bloqueio.")
                    
                    if st.button("⬅️ Voltar ao Início", use_container_width=True):
                        navegar_para("🏠 Início")
                    st.stop()

            # --- FLUXO NORMAL: SE NÃO HOUVER CONFLITO (OU SE FOR TIPO DIFERENTE), SEGUE PARA OS ---
