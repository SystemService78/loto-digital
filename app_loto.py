            ...
            if df_visualizacao.empty:
                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        # --- LÓGICA DE CÁLCULO DE TEMPO DECORRIDO (FIX TOTAL) ---
                        tempo_formatado = "---"
                        try:
                            fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
                            # 1. Pega o "agora" em Brasília e remove o fuso para cálculo puro
                            agora_br = datetime.datetime.now(fuso_br).replace(tzinfo=None)
                            ...
                            if h_ini_str:
                                ...
                                # 2. Se a hora de início parece ser no futuro, ela é de ontem
                                if dt_ini > agora_br:
                                ...
                                # Garante que não exiba negativo por conta de segundos de processamento
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        if user_p['PERFIL'] == 'Perfil System':
            # Admin Geral vê tudo (5 abas)
            tabs = st.tabs(["➕ Novo Usuário", "📝 Editar Usuário", "🛠️ Equipamentos", label_pedidos, "📁 Arquivos/Fotos"])
            tab_cadastro, tab_gerenciamento, tab_equipamentos, tab_senha, tab_arquivos = tabs
        else:
            # Gerencial vê apenas o necessário (4 abas)
            tabs = st.tabs(["➕ Novo Usuário", "📝 Editar Usuário", "📁 Arquivos/Fotos", label_pedidos])
        ...
        with tab_cadastro:
            ...
            with st.container(border=True):
                ...
                c1, c2 = st.columns(2)
                # Inversão: Perfil depois Área
                perfil_sel = c1.selectbox("Perfil *", ["Perfil Operação", "Perfil Gerencial Clientes"])
                if perfil_sel == "Perfil Operação":
                    areas = ["Manutenção Mecânica", "Manutenção Elétrica", "SESMT"]
                else:
                    areas = ["Supervisão Manutenção"]
                
                area_sel = c2.selectbox("Área *", areas)
        ...
        with tab_senha:
            ...
            if not df_pendente.empty:
                ...
                # Exibe cada solicitação como um card com botão próprio
                for idx, row in df_pendente.iterrows():
                    with st.container(border=True):
                        c1, c2, c3 = st.columns([3, 2, 1])
                        with c1:
                            st.markdown(f"**👤 Usuário:** {row['NOME_EMPRESA']}")
                            st.caption(f"📧 {row['EMAIL']} | 🏢 {row.get('EMPRESA', 'Não informada')}")
                        with c2:
                            st.markdown(f"**💬 Obs:** {row['OBSERVACAO']}")
                            st.caption(f"📅 Data: {row['DATA']}")
                        with c3:
                            # Botão individual usando o ID do banco
                            if st.button("✅ Resolver", key=f"res_{row['ID']}"):
                                executar_sql("UPDATE solicitacoes SET STATUS = 'Resolvido' WHERE ID = ?", (row['ID'],))
                                st.rerun()
                
                st.write("##")
                if st.button("Marcar todas como Resolvidas", type="secondary"):
