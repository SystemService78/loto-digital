            ...
            try:
                ...
                if not res_storage:
                    ...
                    if user_p['PERFIL'] == 'Perfil System' and filtro_emp_arq == "Todas":
                        ...
                        # Gerencial ou Master com filtro de empresa específica
                        empresa_alvo = filtro_emp_arq if user_p['PERFIL'] == 'Perfil System' else user_p['EMPRESA']
                        
                        # Descobre quais caminhos de arquivos pertencem a esta empresa no histórico
                        ...
                        if not df_vinc.empty:
                            for obs in df_vinc['OBS_GERAL'].dropna():
                                if "Fotos: " in str(obs):
                                    permitidos.extend([n.strip() for n in str(obs).split("Fotos: ")[-1].split(", ")])
                        
                        # Filtra a lista do storage
                        for item in res_storage:
                            nome = item['name']
                            is_pasta = item.get('id') is None
                            # Se for pasta, vemos se o nome da pasta (OS) está nos registros da empresa
                            # Se for arquivo, vemos se o caminho completo está nos registros
                            caminho_check = f"{st.session_state.pasta_atual}/{nome}" if st.session_state.pasta_atual else nome
                            if any(caminho_check in p for p in permitidos) or is_pasta:
