...
st.markdown("""
    ...
    /* FORÇAR MODO CLARO (Sobrescreve variáveis do Streamlit) */
    :root {
    ...
    /* Bloqueia fundos pretos em containers do modo escuro */
    ...
    /* Forçar transparência em todas as imagens do app */
    img {
        background-color: transparent !important;
    }
    /* Remove sombras ou bordas que o Streamlit coloca em containers de imagem */
            ...
            if df_visualizacao.empty:
                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        # --- LÓGICA DE CÁLCULO DE TEMPO ROBUSTA (DIAS/HORAS/MINUTOS) ---
                        tempo_formatado = "---"
                        try:
                            fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
                            agora_br = datetime.datetime.now(fuso_br).replace(tzinfo=None)
                            ...
                            if h_ini_str:
                                if len(h_ini_str) <= 8: # Formato HH:MM:SS (Assume dia atual)
                                    hoje_str = agora_br.strftime("%d/%m/%Y")
                                    dt_ini = datetime.datetime.strptime(f"{hoje_str} {h_ini_str}", "%d/%m/%Y %H:%M:%S")
                                    # Se o horário de início for "maior" que agora, ele começou ontem
                                    if dt_ini > agora_br:
                                        dt_ini -= datetime.timedelta(days=1)
                                else: # Formato completo DD/MM/YYYY HH:MM
                                    dt_ini = datetime.datetime.strptime(h_ini_str, "%d/%m/%Y %H:%M")
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        if 'aba_admin_ativa' not in st.session_state:
            st.session_state.aba_admin_ativa = opcoes_admin[0]
                    ...
                    with st.container(border=True):
                        ...
                        with c3:
                            ...
                            if st.button("Marcar como resolvido ✅", key=f"btn_sol_{row['ID']}", use_container_width=True):
                                # Aqui enviamos o ID da linha atual (row['ID']) como o parâmetro que a função agora aceita
