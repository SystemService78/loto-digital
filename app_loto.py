...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        # --- ABAS COMUNS (Perfil System e Perfil Gerencial Clientes) ---
            ...
            try:
                # 1. Busca RECURSIVA
                res_storage = conn.client.storage.from_("evidencias").list(options={"recursive": True})
                
                # 2. FILTRAGEM SEGURA: Remove pastas e garante que o arquivo tenha metadados
                # Isso impede o erro de 'NoneType' na ordenação
                lista_valida = [a for a in res_storage if a.get('metadata') and a.get('created_at')]

                # Filtro de Privacidade (Mantém sua lógica)
                if user_p['PERFIL'] != 'Perfil System' or filtro_emp_arq != "Todas":
                    ...
                    arquivos_filtrados = [a for a in lista_valida if a['name'] in fotos_permitidas]
                else:
                    arquivos_filtrados = [a for a in lista_valida if busca_arq.lower() in a['name'].lower()]

                # 3. ORDENAÇÃO (Agora segura, pois todos têm created_at)
                if ordem_arq == "Mais recentes":
                    ...
                    for arq in arquivos_filtrados:
                        ...
                        # PULA PASTAS: Se não tem 'created_at' ou termina com '/', ignora (Resolve o erro NoneType)
                        if not arq.get('created_at') or nome_arq.endswith('/'):
                            continue
