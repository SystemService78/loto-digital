...
st.markdown("""
    ...
    /* 1. FORÇAR FUNDO CLARO EM TUDO (SIDEBAR E CORPO) */
    .stApp, [data-testid="stAppViewContainer"], [data-testid="stHeader"], [data-testid="stSidebar"], [data-testid="stSidebarContent"] {
        background-color: white !important;
    }

    /* 2. FORÇAR TEXTOS ESCUROS EM TODOS OS LUGARES (INCLUSIVE SIDEBAR) */
    h1, h2, h3, p, span, label, li, [data-testid="stMarkdownContainer"] p {
        color: #1E293B !important;
    }
    
    /* 3. FIX ESPECÍFICO PARA OS BOTÕES DE SELEÇÃO (RADIOS/ABAS) NO MODO DARK */
    ...
    /* 4. BLINDAGEM DE CAMPOS DE DIGITAÇÃO E LISTAS */
    ...
    /* 5. ESTILO DOS CARDS DE MÉTRICA (HOME) - CORES RESTAURADAS */
    .metric-container {
        background-color: white !important;
    ...
    .metric-label-custom { font-size: 1.1rem !important; font-weight: 600 !important; margin-bottom: 5px !important; color: #64748B !important; }
    .metric-value-custom { font-size: 3.5rem !important; font-weight: 800 !important; margin-bottom: 0 !important; }

    /* Cores das Métricas com prioridade máxima */
    ...
    /* 7. ALERTA VIBRANTE MANUAL (Caso o st.error falhe no CSS) */
    .alerta-vermelho {
                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            ...
                            if h_ini_str:
                                if len(h_ini_str) <= 8: # HH:MM:SS
                                    hoje_data = agora.strftime("%d/%m/%Y")
                                    dt_ini = datetime.datetime.strptime(f"{hoje_data} {h_ini_str}", "%d/%m/%Y %H:%M:%S")
                                else: # DD/MM/YYYY HH:MM
                                    dt_ini = datetime.datetime.strptime(h_ini_str, "%d/%m/%Y %H:%M")
                                
                                # Pegamos a diferença bruta em segundos
                                diff_seconds = (agora - dt_ini).total_seconds()
                                
                                # O SEGREDO: Se a diferença for muito grande e negativa, 
                                # é porque cruzou a meia-noite (24h = 86400 seg)
                                if diff_seconds < 0:
                                    # Tenta ajustar somando 24 horas se for apenas erro de fuso/virada
                                    diff_seconds += 86400
                                    
                                # Usamos o valor absoluto (módulo) para garantir que nunca exiba "-"
                                total_segundos = abs(int(diff_seconds))
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        st.session_state.aba_admin_ativa = escolha_aba_admin
        st.write("---")
