...
with st.sidebar:
    ...
    st.segmented_control(
        "Navegação:", 
        OPCOES_MENU, 
        key="pagina_ativa", # KEY idêntica à variável faz a troca ser automática e instantânea
    ...
    with st.container(border=True):
        ...
        if setor_sel and setor_sel != "Selecione o setor":
            ...
            if interditada:
                ...
                nome_responsavel = dados_interdicao.get('RESP') or dados_interdicao.get('resp') or str(id_registro_interdicao).split('|')[0]
                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            fuso_br = datetime.timezone(datetime.timedelta(hours=-3))
                            agora_br = datetime.datetime.now(fuso_br).replace(tzinfo=None)
                            h_ini_str = row.get('HORA_INICIO', '') # Aqui usamos row
                            ...
                            if h_ini_str:
                                ...
                                total_sec = abs(int(diff.total_seconds()))
