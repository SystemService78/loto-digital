...
def salvar_dataframe_no_sql(df, tabela):
    """Detecta se é histórico e empacota o checklist em JSONB automaticamente"""
    try:
        df_save = df.copy()
        df_save.columns = [c.lower() for c in df_save.columns]
        
        # Se for a tabela de histórico, vamos empacotar as colunas de checklist
        if tabela.lower() == "historico_loto":
            colunas_fixas = ['data', 'hora_inicio', 'hora_fim', 'os', 'resp', 'setor', 'equip', 'tipo', 'status', 'obs_geral']
            cols_checklist = [c for c in df_save.columns if c not in colunas_fixas]
            
            # Cria a coluna jsonb e remove as originais
            df_save['dados_etapas'] = df_save[cols_checklist].to_dict(orient='records')
...
def executar_sql(query, params=()):
    ...
    try:
        # Identifica a tabela alvo
        tabelas = ['usuarios', 'equipamentos', 'historico_loto', 'solicitacoes', 'ativos_loto']
        tabela_alvo = next((t for t in tabelas if t.upper() in query_up), None)

        if "UPDATE SOLICITACOES" in query_up:
            # Forçamos o 'resolvido' em minúsculo se o banco estiver assim
            conn.table("solicitacoes").update({"status": "Resolvido"}).eq("status", "Pendente").execute()
        
        elif "INSERT INTO SOLICITACOES" in query_up:
        ...
        # ... (mantenha os outros elif de INSERT se necessário)
