                ...
                if info_loto.get('TIPO') == 'SESMT':
                    ...
                    st.markdown(f"""
                        <div class="alerta-vermelho">
                            ⚠️ INTERDIÇÃO SESMT: {maq_at}<br>
                            <span style="font-size: 0.9rem; font-weight: normal;">
                                Tempo de Interdição: {tempo_formatado} | Responsável: {nome_exibir}
                            </span>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            ...
                            if h_ini_str:
                                ...
                                if len(h_ini_str) > 10:
                                    try: dt_ini = datetime.datetime.strptime(h_ini_str, "%d/%m/%Y %H:%M:%S")
                                    except: dt_ini = datetime.datetime.strptime(h_ini_str, "%d/%m/%Y %H:%M")
                                else:
