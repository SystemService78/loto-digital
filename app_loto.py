                ...
                if os_sesmt:
                    if st.button("🚨 INTERDITAR MÁQUINA AGORA", type="primary", use_container_width=True):
                        ...
                        estado_interdicao = {
                            ...
                            "HORA_INICIO": agora_f.strftime("%d/%m/%Y %H:%M")
                        }
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        if user_p['PERFIL'] == 'Perfil System':
            ...
            # Admin Cliente vê três: Cadastro, Edição e Arquivos
            tabs = st.tabs(["➕ Novo Usuário", "📝 Editar Usuário", "📁 Arquivos/Fotos", label_solicitacoes])
            tab_cadastro, tab_gerenciamento, tab_arquivos, tab_senha = tabs
        ...
        if user_p['PERFIL'] == 'Perfil System':
            with tab_cadastro:
                if user_p['PERFIL'] == 'Perfil System': # Proteção extra
                    ...
                    with st.container(border=True):
                        ...
                        # --- BLOCO DA GARANTIA DE EMPRESA FIXA (INSIRA AQUI) ---
                        if user_logado['PERFIL'] == 'Perfil System':
                            # Busca empresas existentes no banco para o System
                            ...
                            modo_empresa = c_emp1.selectbox("Modo Empresa:", ["Selecionar Existente", "Cadastrar Nova"])
                            
                            if modo_empresa == "Selecionar Existente":
                                st.session_state["reg_empresa"] = c_emp2.selectbox("Empresas Cadastradas:", lista_empresas_existentes)
                            else:
                                st.session_state["reg_empresa"] = c_emp2.text_input("Nome da Nova Empresa:", placeholder="Digite aqui...")
                        else:
                            # Perfil Gerencial: Trava na dele
            ...
            with tab_gerenciamento:
                ...
                # --- FILTRO DE PRIVACIDADE NA EDIÇÃO ---
                #df_u = ler_sql("usuarios")
                if user_p['PERFIL'] == 'Perfil Gerencial Clientes':
                    # O Gerencial só vê usuários da mesma empresa que a dele
                    df_u = df_u[df_u['EMPRESA'] == user_p['EMPRESA']]
