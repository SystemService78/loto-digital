...
# --- MÉTRICAS REAIS VIA SQL (FILTRADAS POR PERFIL) ---
    ...
    try:
        ...
        if perfil_logado == 'Admin':
            ...
            # Filtro por Empresa
            empresa_usuario = str(user_atual.get('EMPRESA', '')).strip().upper()
            df_h_count = df_h_tudo[df_h_tudo['EMPRESA'].astype(str).str.upper().str.strip() == empresa_usuario] if not df_h_tudo.empty else pd.DataFrame()
            df_a_count = df_a_tudo[df_a_tudo['EMPRESA'].astype(str).str.upper().str.strip() == empresa_usuario] if not df_a_tudo.empty else pd.DataFrame()
        else:
            # Filtro por Operador
