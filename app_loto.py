...
def ler_sql(query_ou_tabela, params=None):
    ...
    try:
        ...
        if not df_res.empty:
            # 1. Se houver a coluna JSONB 'dados_etapas', nós a "descompactamos"
            # Isso faz com que as 30 colunas de checklist apareçam de volta no DataFrame
            if 'dados_etapas' in df_res.columns:
                df_json = pd.json_normalize(df_res['dados_etapas'])
                df_res = pd.concat([df_res.drop(columns=['dados_etapas']), df_json], axis=1)

            # 2. Força todas as colunas para MAIÚSCULO para o seu código antigo
