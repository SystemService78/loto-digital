...
def ler_sql(query_ou_tabela, params=None):
    ...
    try:
        ...
        # Filtro de leitura (Login, Histórico, Solicitações)
        query_up = query_ou_tabela.upper()
        if "WHERE" in query_up:
            try:
                # Extrai a coluna e o valor. Remove aspas e espaços.
                partes = query_up.split("WHERE")[1].split("=")
                coluna = partes[0].strip().lower()
                # Remove hifens se o código enviar 'e-mail' em vez de 'email'
        ...
        if not df_res.empty:
            # Descompacta o checklist se for histórico
            ...
            # TRADUTOR: Força MAIÚSCULO para o seu código de 1.500 linhas
            df_res.columns = [str(c).strip().upper() for c in df_res.columns]
            # Caso a coluna venha como E-MAIL, padroniza para EMAIL
            df_res = df_res.rename(columns={'E-MAIL': 'EMAIL'})
            
        return df_res
    except Exception as e:
        print(f"Erro Sherlock Leitura: {e}")
        return pd.DataFrame()
...
def executar_sql(query, params=()):
    ...
    try:
        ...
        # --- CASO 2: EQUIPAMENTOS (CADASTRO E EDIÇÃO) ---
        elif tabela_alvo == "equipamentos":
            # Watson, use esses nomes de colunas EXATOS no Supabase
            dados = {
                "setor": params[0], 
            ...
            if "INSERT" in query_up:
                conn.table("equipamentos").insert(dados).execute()
            elif "UPDATE" in query_up:
                # Usa os nomes antigos (params[5] e [6]) para achar a linha certa
                conn.table("equipamentos").update(dados).eq("setor", params[5]).eq("equipamento", params[6]).execute()
            elif "DELETE" in query_up:
                conn.table("equipamentos").delete().eq("setor", params[0]).eq("equipamento", params[1]).execute()

        # --- CASO 3: USUÁRIOS (CADASTRO E EDIÇÃO) ---
            ...
            if "INSERT" in query_up:
                ...
                if "SET SENHA" in query_up:
                    ...
                    # O segredo é garantir que o email (params[4]) não esteja vazio
                    if params[4]:
                        conn.table("usuarios").update(dados).eq("email", params[4]).execute()
            elif "DELETE" in query_up:
