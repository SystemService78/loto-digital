...
try:
    ...
    conf = st.secrets["google_sheets"].to_dict().copy()
    
    # 2. Guardamos a URL em uma variável e a removemos do dicionário de credenciais
    URL_PLANILHA = conf.get("spreadsheet")
    
    # 3. Limpamos a chave privada (converte \\n em quebra de linha real)
    if "private_key" in conf:
        conf["private_key"] = conf["private_key"].replace("\\n", "\n")
    
    # 4. Removemos campos que o st.connection não aceita no dicionário extra
    for k in ["spreadsheet", "type"]:
        if k in conf: conf.pop(k)

    # 5. Criamos a conexão (loto_db)
    conn = st.connection("loto_db", type=GSheetsConnection, **conf)
...
def atualizar_registro_unico(df_completo, aba):
    ...
    try:
        # Atualizamos passando a URL
        conn.update(spreadsheet=URL_PLANILHA, worksheet=aba, data=df_completo)
        st.cache_data.clear()
    except Exception as e:
...
def ler_sql(query, params=()):
    ...
    try:
        # AQUI O SEGREDO: Passamos a URL da planilha DIRETAMENTE no read
        df_gs = conn.read(spreadsheet=URL_PLANILHA, worksheet=tabela_alvo, ttl=0)
        
        if df_gs is None or df_gs.empty:
            # Proteção contra KeyError: Retorna colunas vazias se for Equipamentos
        ...
        if "WHERE" in query_up and params:
            coluna = query_up.split("WHERE")[1].split("=")[0].strip().upper()
            df_gs = df_gs[df_gs[coluna].astype(str).str.lower() == str(params[0]).lower()]
            
        return df_gs
    except Exception as e:
        st.error(f"Erro na leitura da aba {tabela_alvo}: {e}")
        return pd.DataFrame()

def salvar_dataframe_no_sql(df_novo, tabela):
    """Adiciona uma nova linha (Append)"""
    try:
        # Lemos a planilha atual passando a URL
        df_atual = conn.read(spreadsheet=URL_PLANILHA, worksheet=tabela, ttl=0)
        df_final = pd.concat([df_atual, df_novo], ignore_index=True)
        # Salvamos de volta passando a URL
        conn.update(spreadsheet=URL_PLANILHA, worksheet=tabela, data=df_final)
        st.cache_data.clear()
    except Exception as e:
