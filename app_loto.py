...
try:
    # Lemos da seção 'minha_planilha' que criamos no Secrets
    conf = st.secrets["minha_planilha"].to_dict()
    
    # Limpamos a chave privada
    if "private_key" in conf:
        conf["private_key"] = conf["private_key"].replace("\\n", "\n")
    
    # Extraímos a URL e o tipo para não dar conflito de argumentos
    url_gs = conf.pop("spreadsheet")
    
    # Criamos a conexão passando os parâmetros limpos
