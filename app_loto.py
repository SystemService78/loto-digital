...
# --- MÉTRICAS REAIS VIA SQL (FILTRADAS POR PERFIL) ---
    ...
    # Mudança: Usamos o cliente direto para garantir que pegue a lista completa
    ...
    with c_b2:
        if ativos_usuario:
            ...
            for reg_at in ativos_usuario:
                ...
                tipo_at = info_loto.get('TIPO', 'Operação')
                ...
                if progresso_h >= 7:
                    st.error(f"🚨 **ALERTA DE SEGURANÇA:** A máquina **{maq_at}** encontra-se **FORA DE OPERAÇÃO**.")
                # ----------------------------------------------
...
##trecho removido daqui
    ...
    if maquina_sel:
        ...
        with st.container(border=True):
            ...
            # --- ENGENHARIA DE CONFLITOS (PASSO 2 COMPLETO E INTEGRAL) ---
            # 1. Busca se o PRÓPRIO usuário já tem este tipo de serviço nesta máquina
            # Usamos a função ler_ativo_sql que já valida (Dono, Máquina, Tipo)
            conflito_direto = ler_ativo_sql(user_nome, maquina_sel, tipo)
            
            # 2. Busca se algum COLEGA da mesma empresa já tem este tipo de serviço nesta máquina
            res_geral = conn.client.table("ativos_loto").select("*").execute()
            lista_ativos = res_geral.data if res_geral.data else []
            
            conflito_colega = None
            for reg_db in lista_ativos:
                d_json = reg_db.get('dados_json') or reg_db.get('DADOS_JSON')
                if d_json:
                    # Normalização para comparação (Engenharia de Dados robusta)
                    equip_db = str(d_json.get('EQUIP', '')).strip().upper()
                    tipo_db = str(d_json.get('TIPO', '')).strip().upper()
                    empresa_db = str(d_json.get('EMPRESA', '')).strip().upper()
                    
                    maquina_atual = str(maquina_sel).strip().upper()
                    tipo_atual = str(tipo).strip().upper()
                    empresa_atual = str(st.session_state.usuario.get('EMPRESA', '')).strip().upper()
                    
                    # Extrai o nome do responsável para diferenciar Colega de Dono
                    id_resp_db = reg_db.get('resp', '')
                    dono_id = id_resp_db.split('|')[0] if '|' in id_resp_db else id_resp_db

                    # CONDIÇÃO DE CONFLITO COM COLEGA: Mesma Máquina + Mesmo Tipo + Mesma Empresa + DONO DIFERENTE
                    if (equip_db == maquina_atual and 
                        tipo_db == tipo_atual and 
                        empresa_db == empresa_atual and 
                        dono_id != user_nome):
                        conflito_colega = reg_db
                        conflito_colega['DADOS_JSON_FIX'] = d_json
                        break

            # --- LÓGICA DE TOMADA DE DECISÃO (SEGURANÇA E FLUXO) ---
            if st.session_state.editando_agora == False:
                
                # CENÁRIO A: O conflito é do próprio usuário (Hugo vê seus botões de Retomar/Cancelar)
                if conflito_direto:
                    st.warning(f"⚠️ {user_nome}, você já possui um bloqueio de **{tipo}** em aberto para esta máquina.")
                    
                    # Alerta de Segurança: Máquina fora de operação (7/10 itens marcados ou N/A)
                    prog_c = 0
                    ts_c = conflito_direto.get("TIMESTAMPS", {})
                    na_c = conflito_direto.get("NA_ETAPAS", {})
                    for i in range(7):
                        n_it = itens_loto[i][1]
                        if ts_c.get(f"t_{n_it}") or na_c.get(f"na_{n_it}"):
                            prog_c += 1
                    
                    if prog_c >= 7:
                        st.error(f"🚨 **ALERTA DE SEGURANÇA:** A máquina **{maquina_sel}** encontra-se **FORA DE OPERAÇÃO**.")

                    c_ret1, c_ret2 = st.columns(2)
                    
                    if c_ret1.button("▶️ Continuar Meu Trabalho", use_container_width=True, key="btn_retomar_v_final"):
                        # REPOSIÇÃO INTEGRAL DO ESTADO (O Pulo do Gato)
                        st.session_state["os_exec"] = conflito_direto["OS"]
                        st.session_state["setor_exec"] = conflito_direto["SETOR"]
                        st.session_state["maq_exec"] = conflito_direto["EQUIP"]
                        st.session_state["tipo_exec"] = conflito_direto["TIPO"]
                        st.session_state["obs_final_exec"] = conflito_direto.get("OBS_GERAL", "")
                        st.session_state.hora_inicio_loto = conflito_direto["HORA_INICIO"]
                        st.session_state.timestamps = conflito_direto["TIMESTAMPS"]
                        st.session_state.obs_etapas = conflito_direto["OBS_ETAPAS"]
                        st.session_state.na_etapas = conflito_direto["NA_ETAPAS"]
                        
                        # Sincronização de todos os Checkboxes, NAs e Popovers
                        for _, nome_item in itens_loto:
                            v_na = st.session_state.na_etapas.get(f"na_{nome_item}", False)
                            st.session_state[f"na_{nome_item}"] = v_na
                            h_v = st.session_state.timestamps.get(f"t_{nome_item}", "N/A")
                            st.session_state[f"check_{nome_item}"] = (h_v != "N/A") or v_na
                            st.session_state[f"input_{nome_item}"] = st.session_state.obs_etapas.get(nome_item, "")
                        
                        st.session_state.editando_agora = True
                        st.rerun()
                    
                    if c_ret2.button("➕ Iniciar Novo (Descarta antigo)", use_container_width=True, key="btn_novo_v_final"):
                        # REGISTRO DE CANCELAMENTO PARA AUDITORIA
                        dados_p_hist = {
                            "DATA": [datetime.datetime.now().strftime("%d/%m/%Y")],
                            "HORA_INICIO": [conflito_direto.get("HORA_INICIO", "N/A")],
                            "HORA_FIM": [datetime.datetime.now().strftime("%H:%M:%S")],
                            "OS": [conflito_direto.get("OS", "N/A")],
                            "RESP": [user_nome], "SETOR": [setor_sel], "EQUIP": [maquina_sel], "TIPO": [tipo],
                            "STATUS": ["CANCELADO"], "EMPRESA": [st.session_state.usuario.get('EMPRESA', '')],
                            "OBS_GERAL": ["Substituído por nova execução (Cancelamento Automático)"]
                        }
                        for _, n_it in itens_loto:
                            u_n = n_it.upper()
                            dados_p_hist[f"HORA_{u_n}"] = [conflito_direto.get('TIMESTAMPS', {}).get(f"t_{n_it}", "N/A")]
                            dados_p_hist[f"OBS_{u_n}"] = [conflito_direto.get('OBS_ETAPAS', {}).get(n_it, "")]
                            dados_p_hist[f"NA_{u_n}"] = [conflito_direto.get('NA_ETAPAS', {}).get(f"na_{n_it}", False)]

                        salvar_dataframe_no_sql(pd.DataFrame(dados_p_hist), "historico_loto")
                        # Deleta especificamente a chave composta do conflito
                        chave_del = f"{user_nome}|{maquina_sel}|{tipo}"
                        conn.client.table("ativos_loto").delete().eq("resp", chave_del).execute()
                        
                        # Limpeza da memória para o novo formulário vir limpo
                ...
                # CENÁRIO B: O conflito é de um COLEGA (Usuário é bloqueado preventivamente)
                elif conflito_colega:
                    d_col = conflito_colega['DADOS_JSON_FIX']
                    dono_bloqueio = conflito_colega['resp'].split('|')[0]
                    
                    st.error(f"🚫 **BLOQUEIO EXISTENTE:** O colaborador **{dono_bloqueio}** já possui um registro de **{tipo}** para esta máquina.")
                    st.info(f"Para sua segurança, você não pode intervir até que a OS **{d_col.get('OS')}** seja concluída.")
                    
                    # Alerta 7/10 do colega (Segurança Coletiva)
                    c_col = 0
                    ts_col = d_col.get("TIMESTAMPS", {})
                    na_col = d_col.get("NA_ETAPAS", {})
                    for i in range(7):
                        n_it = itens_loto[i][1]
                        if ts_col.get(f"t_{n_it}") or na_col.get(f"na_{n_it}"):
                            c_col += 1
                            
                    if c_col >= 7:
                        st.warning(f"🚨 Atenção: {dono_bloqueio} já encontra-se em estágio crítico de bloqueio nesta máquina.")
                    
                    if st.button("⬅️ Voltar ao Início", use_container_width=True):
                        navegar_para("🏠 Início")
                    st.stop()

            # --- FLUXO NORMAL: EXIBE CAMPOS DE OS SE NÃO HOUVER CONFLITO (OU SE JÁ ESTIVER EDITANDO) ---
