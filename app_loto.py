    ...
    with c_b2:
        if ativos_usuario:
            ...
            for reg_at in ativos_usuario:
                ...
                # --- Notificação de Máquina Interditada pelo SESMT --- #
                if info_loto.get('TIPO') == 'SESMT':
                    st.markdown(f"""
                        <div style="background-color: #FEE2E2; padding: 15px; border-radius: 10px; border-left: 5px solid #EF4444; margin-bottom: 10px;">
                            <b style="color: #991B1B;">⚠️ INTERDIÇÃO ATIVA: {maq_at}</b><br>
                            <small style="color: #B91C1C;">Pelo SESMT: {info_loto.get('RESP')}</small>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        st.markdown(f"""
                            <div class="hist-card" style="border-left-color: #3B82F6;">
                                ...
                                <div class="hist-sub">🕒 Iniciado em: {row.get('HORA_INICIO', 'N/A')} | 👤 {row['RESP']}</div>
                            </div>
                        """, unsafe_allow_html=True)
                    else:
            ...
            with tab_cadastro:
                if user_p['PERFIL'] == 'Perfil System': # Proteção extra
                    ...
                    with st.container(border=True):
                        ...
                        # 1. Seleciona o Perfil primeiro
                        perfil_selecionado = c2.selectbox(
                        ...
                        if perfil_selecionado == "Perfil Operação":
                            ...
                            opcoes_area = ["TI", "Diretoria", "Administrador Master"]

                        # 3. Exibe o selectbox de Área com as opções filtradas
                        area_selecionada = c1.selectbox("Área de Atuação *", opcoes_area)
                        
                        # Injeta nas chaves que o botão de salvar usa
