...
# --- MÉTRICAS REAIS VIA SQL (FILTRADAS POR PERFIL) ---
    ...
    try:
        ...
        if perfil_logado == 'Admin':
            ...
            # Filtro por Empresa
            df_h_count = df_h_tudo[df_h_tudo['EMPRESA'].astype(str).str.strip() == str(empresa_logada).strip()] if not df_h_tudo.empty else pd.DataFrame()
            df_a_count = df_a_tudo[df_a_tudo['EMPRESA'].astype(str).str.strip() == str(empresa_logada).strip()] if not df_a_tudo.empty else pd.DataFrame()
        else:
            # Filtro por Operador
