...
# --- 1. CONEXÃO ---
try:
    # 1. Busca os dados da seção que criamos no painel
    if "google_sheets" not in st.secrets:
        st.error("Erro: A seção [google_sheets] não foi encontrada nos Secrets do painel.")
        st.stop()
        
    creds_dict = st.secrets["google_sheets"].to_dict().copy()
    
    # 2. Salva a URL e limpa o dicionário para a conexão não reclamar
    URL_PLANILHA = creds_dict.pop("spreadsheet")
    
    # 3. Limpa a Private Key (Trata o problema dos caracteres zangados)
    if "private_key" in creds_dict:
        creds_dict["private_key"] = creds_dict["private_key"].replace("\\n", "\n")

    # 4. Inicia a conexão passando os parâmetros limpos
    # Note que não passamos 'type=' aqui para evitar o erro de múltiplos valores
