...
##trecho removido daqui
    ...
    if maquina_sel:
        ...
        with st.container(border=True):
            ...
            # --- RADAR DE CONFLITO EMPRESARIAL (VALIDAÇÃO SENIOR) ---
            # 1. Buscamos todos os ativos da empresa para garantir segurança coletiva
            ...
            for reg in lista_ativos:
                # Normalização: Trata se o banco retornar 'dados_json' ou 'DADOS_JSON'
                ...
                if d:
                    ...
                    m_atual = str(maquina_sel).strip().upper()
                    t_atual = str(tipo).strip().upper()
                    e_atual = str(st.session_state.usuario.get('EMPRESA', '')).strip().upper()

                    # VALIDAÇÃO: Máquina + Tipo + Empresa coincidem?
                    if m_db == m_atual and t_db == t_atual and e_db == e_atual:
                        conflito_detectado = reg
                        conflito_detectado['DADOS_FIX'] = d # Padroniza para uso abaixo
                        break

            # 2. SE HOUVER CONFLITO (e o usuário não estiver vindo de uma retomada da Home)
            if conflito_detectado and st.session_state.editando_agora == False:
                dados_c = conflito_detectado['DADOS_FIX']
                # Extrai o nome do dono do bloqueio (ID 'resp' é "Nome|Máquina|Tipo")
                id_resp_db = conflito_detectado.get('resp', '')
                dono_bloqueio = id_resp_db.split('|')[0] if '|' in id_resp_db else id_resp_db

                # --- ALERTA DE SEGURANÇA 7/10 (APARECE PARA DONO OU COLEGAS) ---
                prog_7 = 0
                ts_c = dados_c.get("TIMESTAMPS", {})
                nas_c = dados_c.get("NA_ETAPAS", {})
                for i in range(7): # Verifica itens 1 a 7
                    n_it = itens_loto[i][1]
                    if ts_c.get(f"t_{n_it}") or nas_c.get(f"na_{n_it}"):
                        prog_7 += 1
                
                if prog_7 >= 7:
                    st.error(f"🚨 **ALERTA DE SEGURANÇA:** A máquina **{maquina_sel}** encontra-se **FORA DE OPERAÇÃO** (Bloqueio em estágio avançado por {dono_bloqueio}).")

                # CENÁRIO A: O bloqueio é do PRÓPRIO Hugo
                if dono_bloqueio == user_nome:
                    st.warning(f"⚠️ {user_nome}, você já possui um bloqueio de **{tipo}** em aberto para esta máquina.")
                    
                    c_ret1, c_ret2 = st.columns(2)
                    
                    if c_ret1.button("▶️ Continuar Meu Trabalho", use_container_width=True, key="btn_ret_hug_v9"):
                        # REPOSIÇÃO INTEGRAL (Pulo do Gato)
                        st.session_state["os_exec"] = dados_c.get("OS")
                        st.session_state["setor_exec"] = dados_c.get("SETOR")
                        st.session_state["maq_exec"] = dados_c.get("EQUIP")
                        st.session_state["tipo_exec"] = dados_c.get("TIPO")
                        st.session_state["obs_final_exec"] = dados_c.get("OBS_GERAL", "")
                        st.session_state.hora_inicio_loto = dados_c.get("HORA_INICIO")
                    ...
                    if c_ret2.button("➕ Iniciar Novo (Descarta antigo)", use_container_width=True, key="btn_nov_hug_v9"):
                        # REGISTRO DE CANCELAMENTO (Histórico)
                        dados_h = {
                            "DATA": [datetime.datetime.now().strftime("%d/%m/%Y")],
                            "HORA_INICIO": [dados_c.get("HORA_INICIO", "N/A")],
                            "HORA_FIM": [datetime.datetime.now().strftime("%H:%M:%S")],
                            "OS": [dados_c.get("OS", "N/A")], "RESP": [user_nome], "SETOR": [setor_sel],
                            "EQUIP": [maquina_sel], "TIPO": [tipo], "STATUS": ["CANCELADO"],
                            "EMPRESA": [e_atual], "OBS_GERAL": ["Substituído por novo bloqueio"]
                        }
                        for _, n_it in itens_loto:
                            u_n = n_it.upper()
                            dados_h[f"HORA_{u_n}"] = [dados_c.get('TIMESTAMPS', {}).get(f"t_{n_it}", "N/A")]
                            dados_h[f"OBS_{u_n}"] = [dados_c.get('OBS_ETAPAS', {}).get(n_it, "")]
                            dados_h[f"NA_{u_n}"] = [dados_c.get('NA_ETAPAS', {}).get(f"na_{n_it}", False)]

                        salvar_dataframe_no_sql(pd.DataFrame(dados_h), "historico_loto")
                        # Deleta especificamente o ID do conflito
                        conn.client.table("ativos_loto").delete().eq("resp", id_resp_db).execute()
                        
                        # Limpa memória técnica
                ...
                # CENÁRIO B: O bloqueio é de um COLEGA (Bloqueio preventivo)
                else:
                    st.error(f"🚫 **BLOQUEIO EXISTENTE:** O colaborador **{dono_bloqueio}** já possui um registro de **{tipo}** para esta máquina.")
                    st.info(f"Para sua segurança, você não pode intervir até que a OS **{dados_c.get('OS')}** seja concluída.")
                    if st.button("⬅️ Voltar ao Início", use_container_width=True):
                        navegar_para("🏠 Início")
                    st.stop()

            # --- FLUXO NORMAL (SÓ CHEGA AQUI SE NÃO HOUVER CONFLITO) ---
