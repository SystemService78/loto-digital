...
# --- 1. CONEXÃO MANUAL E SEGURA ---
# O Streamlit Cloud lerá automaticamente a seção [connections.gsheets] dos Secrets
#conn = st.connection("gsheets", type=GSheetsConnection)

try:
    # 1. Pegamos os dados da seção 'google_sheets' (ignora a conexão automática problemátiva)
    raw_creds = st.secrets["google_sheets"].to_dict()
    
    # 2. SANITIZAÇÃO DA CHAVE: Removemos literais \\n e forçamos quebras reais
    if "private_key" in raw_creds:
        raw_creds["private_key"] = raw_creds["private_key"].replace("\\n", "\n")
    
    # 3. Extraímos a URL e limpamos o dicionário para o Google Auth
    url_gs = raw_creds.pop("spreadsheet")
    if "type" in raw_creds:
        # Deletamos o 'type' para não conflitar com o parâmetro da função st.connection
        del raw_creds["type"]
    
    # 4. Criamos a conexão loto_db injetando os parâmetros limpos
    conn = st.connection("loto_db", type=GSheetsConnection, spreadsheet=url_gs, **raw_creds)

except Exception as e:
    st.error(f"Erro crítico na conexão: {e}. Verifique os Secrets.")
    st.stop()

# --- PROTEÇÃO CONTRA KEYERROR 'SETOR' ---
# Função para garantir que o DF tenha as colunas mínimas caso a conexão falhe ou aba esteja vazia
def garantir_colunas(df_input, colunas_esperadas):
    if df_input.empty:
        return pd.DataFrame(columns=[c.upper() for c in colunas_esperadas])
    return df_input



# --- BLOCO DE DIAGNÓSTICO (APAGAR DEPOIS) ---
with st.sidebar.expander("DEBUG - Teste de Dados"):
    if st.button("Verificar Conexão"):
        try:
            # Tenta ler a aba usuarios sem filtro nenhum
            teste_df = conn.read(worksheet="usuarios", ttl=0)
            st.write("Colunas encontradas:", teste_df.columns.tolist())
            st.write("Total de linhas:", len(teste_df))
            st.dataframe(teste_df.head())
        except Exception as e:
            st.error(f"Erro ao ler: {e}")
# --------------------------------------------

# --- FERRAMENTAS DO BANCO DE DADOS ---
def atualizar_registro_unico(df_completo, aba):
    """Sobrescreve a aba inteira."""
    try:
        # Note que não passamos mais a URL aqui, ele pega do segredo
        conn.update(worksheet=aba, data=df_completo)
        st.cache_data.clear()
    except Exception as e:
        st.error(f"Erro ao atualizar planilha {aba}: {e}")

def ler_sql(query, params=()):
    tabelas = ['usuarios', 'equipamentos', 'historico_loto', 'solicitacoes', 'ativos_loto']
    tabela_alvo = None
    query_up = query.upper() # <--- Definindo agora corretamente!

    # 1. Identifica qual aba o código quer ler
    for t in tabelas:
        if t.upper() in query_up:
            tabela_alvo = t
            break
    if not tabela_alvo: 
        return pd.DataFrame()
    try:
        # Nota: Agora usamos o 'conn' que foi criado acima
        df_gs = conn.read(worksheet=tabela_alvo, ttl=0)
        
        # Se voltar vazio, criamos as colunas na mão para o app não dar KeyError
        if df_gs is None or df_gs.empty:
            if tabela_alvo == 'equipamentos':
                return pd.DataFrame(columns=['SETOR', 'EQUIPAMENTO', 'LINK', 'LINK_ARQUIVO1', 'LINK_ARQUIVO2'])
            return pd.DataFrame()

        df_gs.columns = [str(c).strip().upper() for c in df_gs.columns]
        # ... (restante da sua lógica de WHERE) ...
        return df_gs
    except:
        return pd.DataFrame()

def salvar_dataframe_no_sql(df_novo, tabela):
    """Adiciona uma nova linha na planilha (Append)"""
    try:
        # 1. Lê os dados que já existem
        df_atual = conn.read(worksheet=tabela, ttl=0)
        ...
        # 3. Manda de volta para o Google
        conn.update(worksheet=tabela, data=df_final)
        st.cache_data.clear() # Limpa a memória para os dados aparecerem
        return True
    except Exception as e:
        st.error(f"Erro ao salvar dados no Google: {e}")
        return False

def executar_sql(query, params=()):
    query_up = query.upper()
    
    # --- NOVO: Suporte para INSERT de usuários e outros ---
    if "INSERT INTO" in query_up:
        # Tenta descobrir em qual tabela inserir
        for tab in ['usuarios', 'solicitacoes', 'equipamentos', 'historico_loto']:
            if tab.upper() in query_up:
                # Transforma os params em um DataFrame de uma linha
                # Note: Isso assume que a ordem dos parâmetros no INSERT bate com as colunas da planilha
                df_u = ler_sql(f"SELECT * FROM {tab} LIMIT 1")
                novo_registro = pd.DataFrame([params], columns=df_u.columns.tolist())
                salvar_dataframe_no_sql(novo_registro, tab)
                break

    elif "DELETE FROM ATIVOS_LOTO" in query_up:
        df_a = conn.read(worksheet="ativos_loto", ttl=0)
        if not df_a.empty:
            df_a.columns = df_a.columns.str.strip().str.upper()
            df_a = df_a[df_a['RESP'] != params[0]]
            atualizar_registro_unico(df_a, "ativos_loto")

    elif "UPDATE SOLICITACOES" in query_up:
        df_s = conn.read(worksheet="solicitacoes", ttl=0)
        if not df_s.empty:
            df_s['STATUS'] = 'Resolvido'
            atualizar_registro_unico(df_s, "solicitacoes")

# --- NOVAS FUNÇÕES PARA SALVAMENTO EM TEMPO REAL ---
def salvar_ativo_sql(resp, dados_dict):
    """Salva o progresso em tempo real no Google Sheets"""
    df = conn.read(worksheet="ativos_loto", ttl=0)
    ...
    # Adiciona o novo
    novo_at = pd.DataFrame([{"RESP": resp, "DADOS_JSON": dados_json}])
    df_final = pd.concat([df, novo_at], ignore_index=True)
    conn.update(worksheet="ativos_loto", data=df_final)

def ler_ativo_sql(resp):
    """Lê o progresso do Google Sheets"""
    df = ler_sql("SELECT DADOS_JSON FROM ativos_loto WHERE RESP = ?", (resp,))
    if not df.empty:
        return json.loads(df.iloc[0]['DADOS_JSON'])
    return None

# --- 3. CARGA GLOBAL ---
# Substitua o seu bloco de carga global por este:
try:
    df = conn.read(worksheet="equipamentos", ttl=600)
    df.columns = df.columns.str.strip().str.upper()
except:
    df = pd.DataFrame(columns=['SETOR', 'EQUIPAMENTO', 'LINK', 'LINK_ARQUIVO1', 'LINK_ARQUIVO2'])

# --- CARGA GLOBAL DE EQUIPAMENTOS ---
def carregar_df_global():
    df_temp = ler_sql("SELECT * FROM equipamentos")
    # Se o banco estiver vazio ou a tabela não existir, criamos a estrutura na memória
    if df_temp.empty or 'SETOR' not in df_temp.columns:
        return pd.DataFrame(columns=['SETOR', 'EQUIPAMENTO', 'LINK', 'LINK_ARQUIVO1', 'LINK_ARQUIVO2'])
    return df_temp

# Atribui o resultado à variável global que o Passo 1 usa
df = carregar_df_global()
...
if st.session_state.pagina_ativa == "🏠 Início":
    ...
    with c_b2:
        if registro_pendente is not None:
            # --- AJUSTE AQUI: Abrimos o pacote JSON para ler os dados ---
            import json
            info_loto = json.loads(registro_pendente['DADOS_JSON'])
            
            # Agora usamos 'info_loto' em vez de 'registro_pendente'
            st.warning(f"Você possui uma execução em aberto: **{info_loto['EQUIP']}**")
            
            if st.button(f"▶️ Continuar Trabalho (OS: {info_loto['OS']})", type="secondary", use_container_width=True):
    ...
    if maquina_sel:
        ...
        if os_n:
            ...
            with st.container(border=True):
                ...
                l_mapa = str(linha_d.get('LINK_ARQUIVO1', '')).strip()
                l_proc = str(linha_d.get('LINK_ARQUIVO2', '')).strip()
                l_raiz = str(linha_d.get('LINK', '')).strip()
                ...
                with st.form("form_novo_equipamento", clear_on_submit=True):
                    ...
                    if st.form_submit_button("✅ Salvar Novo Equipamento"):
                        if new_setor and new_equip:
                            ...
                            query_insert_maq = """
                                INSERT INTO equipamentos (SETOR, EQUIPAMENTO, LINK, LINK_ARQUIVO1, LINK_ARQUIVO2) 
                                VALUES (?, ?, ?, ?, ?)
                            """
            ...
            if not df_maq_edit.empty:
                ...
                if selecao_maq:
                    ...
                    with st.container(border=True):
                        ...
                        up_link_pasta = st.text_input("Alterar Link da Pasta", value=str(dados_atuais['LINK']))
                        up_link_mapa = st.text_input("Alterar Link do Mapa", value=str(dados_atuais['LINK_ARQUIVO1']))
                        up_link_tec = st.text_input("Alterar Link Técnico", value=str(dados_atuais['LINK_ARQUIVO2']))
                        
                        # --- BOTÃO DE SALVAR ALTERAÇÕES NO SQL ---
                        if st.button("💾 Aplicar Alterações no Equipamento", type="primary", use_container_width=True):
                            query_update_maq = """
                                UPDATE equipamentos 
                                SET SETOR = ?, EQUIPAMENTO = ?, LINK = ?, LINK_ARQUIVO1 = ?, LINK_ARQUIVO2 = ?
