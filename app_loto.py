...
def ler_sql(query_ou_tabela, params=None):
    ...
    try:
        ...
        if params:
            if "email" in query_ou_tabela.lower(): query = query.eq("email", params[0])
            elif "resp" in query_ou_tabela.lower(): query = query.eq("resp", params[0])
        
        res = query.execute()
        df_res = pd.DataFrame(res.data) if res.data else pd.DataFrame()
        
        if not df_res.empty:
            # Força colunas em MAIÚSCULO para o seu código de 1.500 linhas não quebrar
