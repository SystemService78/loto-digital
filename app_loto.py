...
# --- 1. CONEXÃO ---
# 1. Pegamos uma CÓPIA dos segredos para podermos modificar sem erro
try:
    gs_creds = st.secrets["connections"]["gsheets"].to_dict()

    # 2. Corrigimos a chave privada na CÓPIA (remove as barras extras do Windows/OneDrive)
    if "private_key" in gs_creds:
        gs_creds["private_key"] = gs_creds["private_key"].replace("\\n", "\n")

    # 3. IMPORTANTE: Removemos o 'type' da cópia para não conflitar com o 
    # parâmetro 'type=' da função st.connection abaixo
    if "type" in gs_creds:
        del gs_creds["type"]

    # 4. Agora sim, conectamos usando o dicionário limpo (**kwargs)
    conn = st.connection("gsheets", type=GSheetsConnection, **gs_creds)

except Exception as e:
    st.error(f"Erro ao carregar as chaves do Google: {e}")
    st.stop()
