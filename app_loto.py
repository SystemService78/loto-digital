...
try:
    # Lemos da seção 'minha_planilha' que você criou no Secrets
    conf = st.secrets["minha_planilha"].to_dict()
    
    # 1. Limpamos a chave privada (corrige as barras do Windows/OneDrive)
    if "private_key" in conf:
        conf["private_key"] = conf["private_key"].replace("\\n", "\n")
    
    # 2. Extraímos a URL para uma variável
    url_gs = conf.pop("spreadsheet")

    # 3. AQUI O PULO DO GATO: Removemos o 'type' do Google para não dar conflito
    # O Google usa type="service_account", mas o Streamlit usa type para a classe da conexão.
    if "type" in conf:
        del conf["type"]
    
    # 4. Criamos a conexão passando os parâmetros limpos
    conn = st.connection("gsheets", type=GSheetsConnection, spreadsheet=url_gs, **conf)
