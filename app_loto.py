...
def ler_sql(query_ou_tabela, params=None):
    """
    Se for apenas o nome da tabela, lê tudo. 
    Se houver params, faz o filtro.
    """
    # Se a query contém 'usuarios', 'equipamentos' etc, extraímos o nome da tabela
    tabelas = ['usuarios', 'equipamentos', 'historico_loto', 'solicitacoes', 'ativos_loto']
    tabela_alvo = next((t for t in tabelas if t in query_ou_tabela.lower()), query_ou_tabela)

    try:
        if params and "email" in query_ou_tabela.lower():
            # Exemplo: Login
            res = conn.table(tabela_alvo).select("*").eq("email", params[0]).execute()
        elif params and "resp" in query_ou_tabela.lower():
            # Exemplo: Busca de ativos
            res = conn.table(tabela_alvo).select("*").eq("resp", params[0]).execute()
        else:
            # Lê a tabela toda
            res = conn.table(tabela_alvo).select("*").execute()
            
        return pd.DataFrame(res.data) if res.data else pd.DataFrame()
    except Exception as e:
        st.error(f"Erro ao ler {tabela_alvo}: {e}")
        return pd.DataFrame()
    
def salvar_registro(tabela, dados_dict):
    # Insere um dado novo
    return conn.table(tabela).insert(dados_dict).execute()

def salvar_dataframe_no_sql(df, tabela):
    """Insere dados novos no Supabase"""
    try:
        dados_dict = df.to_dict(orient='records')
        conn.table(tabela).insert(dados_dict).execute()
        st.cache_data.clear()
    except Exception as e:
        st.error(f"Erro ao salvar em {tabela}: {e}")

def executar_sql(query, params=()):
    """Lida com Deletes e Updates"""
    query_up = query.upper()
    try:
        if "DELETE FROM ATIVOS_LOTO" in query_up:
            conn.table("ativos_loto").delete().eq("resp", params[0]).execute()
        elif "UPDATE SOLICITACOES" in query_up:
            conn.table("solicitacoes").update({"status": "Resolvido"}).eq("status", "Pendente").execute()
        elif "INSERT INTO USUARIOS" in query_up:
            # Adaptador rápido para o seu botão de cadastro
            dados = {"email": params[0], "senha": params[1], "nome": params[2], "area": params[3], "perfil": params[4], "ativo": params[5]}
            conn.table("usuarios").insert(dados).execute()
        st.cache_data.clear()
    except Exception as e:
...
def salvar_ativo_sql(resp, dados_dict):
    # O Supabase salva o dicionário direto na coluna JSONB!
