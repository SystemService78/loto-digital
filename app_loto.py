...
st.markdown("""
    ...
    /* Força verticalização total na Sidebar */
    [data-testid="stSidebar"] [data-testid="stSegmentedControl"] {
        display: flex !important;
        flex-direction: column !important; /* Força ficar um embaixo do outro */
        width: 100% !important;
    }
    ...
    div[data-baseweb="input"], div[data-baseweb="select"], .stSelectbox div, .stTextInput div {
        ...
        background-color: transparent !important;
        outline: none !important; /* Remove o retângulo de foco */
    }
...
with st.sidebar:
    ...
    # Navegação por botões segmentados (Vertical)
    # Seletor único com KEY diferente da variável de estado para evitar erro
    escolha = st.segmented_control(
        "Navegação:", 
        OPCOES_MENU, 
        default=st.session_state.pagina_ativa,
        selection_mode="single",
        key="nav_control_sidebar" 
    )
    
    # Se o usuário clicar em uma opção diferente da atual, ele navega
    if escolha and escolha != st.session_state.pagina_ativa:
                ...
                if info_loto.get('TIPO') == 'SESMT':
                    st.markdown(f"""
                        <div style="background-color: #FEE2E2; padding: 15px; border-radius: 10px; border-left: 5px solid #EF4444; margin-bottom: 10px;">
                            ...
                            <span style="color: #B91C1C;">Tempo: <b>{tempo_formatado}</b></span>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        try:
                            ...
                            if h_ini_str:
                                ...
                                # CALCULO EM SEGUNDOS
                                diff_segundos = (agora_br - dt_ini).total_seconds()
                                
                                # PROVA REAL: Se diff_segundos for negativo, somamos 24 horas (86400 segundos)
                                # Isso resolve o erro de quem começou ontem e terminou hoje
                                if diff_segundos < 0:
                                    diff_segundos += 86400
                                
                                # Se ainda assim for negativo (erro de data futura), travamos em zero
                                total_sec = max(0, int(diff_segundos))
                                
                                horas = total_sec // 3600
                                minutos = (total_sec % 3600) // 60
                                tempo_formatado = f"{horas}h {minutos}min"
                                
                                # --- LINHA DE DEBUG (DESCOMENTE PARA VER A CONTA MANUALMENTE) ---
                                # st.write(f"DEBUG: Início: {dt_ini} | Agora: {agora_br} | Segundos: {total_sec}")
                        except Exception as e:
