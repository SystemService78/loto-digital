    ...
    if maquina_sel:
        ...
        with st.container(border=True):
            ...
            for reg_db in lista_ativos:
                d_json = reg_db['DADOS_JSON']
                # Verificamos se: Mesma Máquina E Mesmo Tipo E Mesma Empresa
                if (str(d_json['EQUIP']) == str(maquina_sel) and 
                    str(d_json['TIPO']) == str(tipo) and 
                    str(d_json.get('EMPRESA')) == str(st.session_state.usuario.get('EMPRESA'))):
                    conflito_v5 = reg_db
                    break

            # Se houver conflito e não estivermos já no meio de uma edição (vindo da Home)
            if conflito_v5 and st.session_state.editando_agora == False:
                dados_retomada = conflito_v5['DADOS_JSON']
                # A chave primária (resp) contém "Nome|Máquina|Tipo"
                dono_bloqueio = conflito_v5['resp'].split('|')[0] 
