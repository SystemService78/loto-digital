...
st.markdown("""
    <style>
    /* 1. Estilo do Card (O Botão Invisível) */
    .metric-card-btn button {
        ...
        font-size: 3.5rem !important; /* Tamanho do Número */
        font-weight: 800 !important;
        display: flex !important;
        flex-direction: column !important;
        align-items: center !important;
        justify-content: center !important;
        padding-top: 40px !important; /* Espaço para o rótulo */
    }
    ...
    /* 2. Inserção dos Rótulos via CSS (Para evitar o bug do texto) */
    .card-total button::before { content: "Total"; }
    .card-exec button::before  { content: "Em Execução"; }
    .card-conc button::before  { content: "Concluídas"; }
    .card-canc button::before  { content: "Canceladas"; }

    /* Estilo do Rótulo (Texto cinza em cima) */
    .card-total button::before, .card-exec button::before, 
    .card-conc button::before, .card-canc button::before {
        position: absolute;
        top: 25px;
        font-size: 1.2rem;
        color: #64748B;
    ...
    /* 3. Cores dos Números */
    .card-total button { color: #1E3A8A !important; } /* Azul */
    .card-exec button  { color: #3B82F6 !important; } /* Azul Brilhante */
    .card-conc button  { color: #10B981 !important; } /* Verde */
    .card-canc button  { color: #EF4444 !important; } /* Vermelho */
...
if st.session_state.pagina_ativa == "🏠 Início":
    ...
    # --- 3. MÉTRICAS CLICÁVEIS (VISUAL ST.METRIC) ---
    st.write("##")
    c1, c2, c3, c4 = st.columns(4)

    with c1:
        st.markdown('<div class="metric-card-btn card-total">', unsafe_allow_html=True)
        if st.button(f"{total_finalizados + total_ativos + total_cancelados}", key="btn_h_total"):
            st.session_state.aba_desejada = "Todas"
            navegar_para("📋 Minhas Execuções")
        st.markdown('</div>', unsafe_allow_html=True)

    with c2:
        st.markdown('<div class="metric-card-btn card-exec">', unsafe_allow_html=True)
        if st.button(f"{total_ativos}", key="btn_h_ativos"):
            st.session_state.aba_desejada = "Ativas"
            navegar_para("📋 Minhas Execuções")
        st.markdown('</div>', unsafe_allow_html=True)

    with c3:
        st.markdown('<div class="metric-card-btn card-conc">', unsafe_allow_html=True)
        if st.button(f"{total_finalizados}", key="btn_h_conc"):
            st.session_state.aba_desejada = "Concluídas"
            navegar_para("📋 Minhas Execuções")
        st.markdown('</div>', unsafe_allow_html=True)

    with c4:
        st.markdown('<div class="metric-card-btn card-canc">', unsafe_allow_html=True)
        if st.button(f"{total_cancelados}", key="btn_h_canc"):
