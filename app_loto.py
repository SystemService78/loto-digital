...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        # --- ABAS COMUNS (Perfil System e Perfil Gerencial Clientes) ---
            ...
            try:
                # Botão para voltar se estiver dentro de uma pasta
                if st.session_state.pasta_atual != "":
                    if st.button(f"⬅️ Voltar para a Raiz", use_container_width=True):
                        st.session_state.pasta_atual = ""
                        st.rerun()

                # Lista o conteúdo do caminho atual (se pasta_atual for "", lista a raiz)
                res_storage = conn.client.storage.from_("evidencias").list(path=st.session_state.pasta_atual)
                
                if not res_storage:
                    st.info("Esta pasta está vazia.")
                else:
                    # Organiza: primeiro pastas (sem id), depois arquivos
                    itens_ordenados = sorted(res_storage, key=lambda x: (x.get('id') is not None, x['name']))
                    
                    cols = st.columns(4)
                    for idx, item in enumerate(itens_ordenados):
                        nome = item['name']
                        is_pasta = item.get('id') is None # Pastas não têm ID no Supabase List
                        
                        # Filtro de busca por nome (se houver algo digitado)
                        ...
                        with cols[idx % 4]:
                            with st.container(border=True):
                                if is_pasta:
                                    st.write(f"📁 **{nome}**")
                                    if st.button("Abrir", key=f"open_{nome}_{idx}"):
                                    ...
                                    if c_del.button("🗑️", key=f"del_{nome}_{idx}"):
