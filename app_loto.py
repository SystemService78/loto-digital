    ...
    if not df_hist.empty:
        ...
        if st.session_state.id_os_detalhe is not None:
            ...
            if status_atual == "Todas":
                qtd_resumo = len(df_filtrado) + len(df_andamento)
                df_para_excel = df_filtrado # Mantém histórico no excel
            elif status_atual == "Ativas":
                qtd_resumo = len(df_andamento)
                df_para_excel = df_andamento
            elif status_atual == "Concluídas":
                df_aux = df_filtrado[df_filtrado['STATUS'] == "CONCLUÍDO"]
                qtd_resumo = len(df_aux)
                df_para_excel = df_aux
            else: # Canceladas
                df_aux = df_filtrado[df_filtrado['STATUS'] == "CANCELADO"]
                qtd_resumo = len(df_aux)
                df_para_excel = df_aux
