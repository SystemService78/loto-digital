...
# --- 1. CONEXÃO ---
try:
    # 1. Pegamos os dados da seção 'google_sheets'
    gs_info = st.secrets["google_sheets"].to_dict().copy()
    
    # 2. Corrigimos a chave privada
    if "private_key" in gs_info:
        gs_info["private_key"] = gs_info["private_key"].replace("\\n", "\n")
    
    # 3. Extraímos a URL e limpamos o dicionário
    url_final = gs_info.pop("spreadsheet", None)

    # 4. AQUI ESTÁ O SEGREDO: 
    # Passamos o GSheetsConnection sem o nome "type=". 
    # Assim o Python não confunde com o "type" que está dentro do gs_info.
    conn = st.connection("loto_db", GSheetsConnection, spreadsheet=url_final, **gs_info)
