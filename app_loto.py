...
# --- MÉTRICAS REAIS VIA SQL (FILTRADAS POR PERFIL) ---
    ...
    try:
        # 1. Baixamos os dados brutos (Sem WHERE para não bugar a função ler_sql)
        df_h_tudo = ler_sql("historico_loto")
        df_a_tudo = ler_sql("ativos_loto")

        # 2. Filtramos usando o Pandas (Igual você faz na tela de listagem)
        if perfil_logado == 'Admin':
            df_h_count = df_h_tudo
            df_a_count = df_a_tudo
        elif perfil_logado == 'Admin Cliente':
            # Filtra pela empresa do Jorge
            df_h_count = df_h_tudo[df_h_tudo['EMPRESA'] == empresa_logada] if not df_h_tudo.empty else pd.DataFrame()
            df_a_count = df_a_tudo[df_a_tudo['EMPRESA'] == empresa_logada] if not df_a_tudo.empty else pd.DataFrame()
        else:
            # Filtra pelo nome do Operador
            df_h_count = df_h_tudo[df_h_tudo['RESP'] == nome_logado] if not df_h_tudo.empty else pd.DataFrame()
            df_a_count = df_a_tudo[df_a_tudo['RESP'] == nome_logado] if not df_a_tudo.empty else pd.DataFrame()
...
# --- 7. HISTÓRICO E DETALHAMENTO ---
    ...
    if not df_hist.empty:
        ...
        if user_atual['PERFIL'] == 'Admin Cliente':
            df_hist = df_hist[df_hist['EMPRESA'] == user_atual['EMPRESA']]
