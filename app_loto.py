    ...
    if not df_hist.empty:
        ...
        if st.session_state.id_os_detalhe is not None:
            ...
            if not df_andamento_raw.empty:
                # 1. Transforma o JSON em colunas para podermos filtrar
                df_andamento = pd.concat([
                    df_andamento_raw.drop(columns=['DADOS_JSON']), 
                    pd.json_normalize(df_andamento_raw['DADOS_JSON'])
                ], axis=1)
                df_andamento['STATUS'] = "EM EXECUÇÃO"

                # 2. Filtro de Empresa/Perfil (Igual ao Histórico)
                if user_atual['PERFIL'] == 'Admin':
                    if st.session_state.f_empresa != "Todas":
                ...
                # 3. FILTROS MANUAIS (Onde estava o erro: agora filtramos as Ativas também)
                if st.session_state.f_os:
                ...
                if st.session_state.f_maq != "Todas":
                    # Nas ativas a coluna de máquina se chama 'EQUIP'
                    df_andamento = df_andamento[df_andamento['EQUIP'] == st.session_state.f_maq]

            # --- BLOCO DE DOWNLOAD (EXCEL) DINÂMICO ---
            status_atual = st.session_state.get('aba_desejada', "Todas")
            
            if status_atual == "Todas":
                # Une os dois dataframes já filtrados
                df_para_excel = pd.concat([df_filtrado, df_andamento], axis=0, ignore_index=True)
                qtd_resumo = len(df_para_excel)
                
            elif status_atual == "Ativas":
                df_para_excel = df_andamento
                qtd_resumo = len(df_para_excel)
