...
st.markdown("""
    ...
    /* 1. FORÇAR MODO CLARO EM TUDO */
    [data-testid="stAppViewContainer"], [data-testid="stHeader"], [data-testid="stSidebar"], .stApp {
        background-color: #F8F9FA !important;
        color: #1E293B !important;
    }

    /* 2. FORÇAR FUNDO BRANCO EM INPUTS E BOTÕES DE ABA (RESOLVE O PRETO NO MODO DARK) */
    input, textarea, select, [data-baseweb="select"], [data-baseweb="input"], [data-testid="stWidgetLabel"] {
        background-color: white !important;
        color: #1E293B !important;
    }
    
    /* Corrigir botões de rádio (Abas) */
    [data-testid="stMarkdownContainer"] p { color: #1E293B !important; }

    /* 3. CORES DAS MÉTRICAS (ESPECIFICIDADE MÁXIMA PARA NÃO FICAR PRETO) */
    .metric-container .metric-label-custom { color: #64748B !important; }
    ...
    .metric-label-custom { font-size: 1.1rem !important; font-weight: 600 !important; margin-bottom: 5px !important; }
    .metric-value-custom { font-size: 3.5rem !important; font-weight: 800 !important; margin-bottom: 10px !important; }

    /* Estilo para o botão "Ver Detalhes" centralizado */
    ...
    /* CORREÇÃO PARA TRANSPARÊNCIA DE PNG */
            ...
            if df_visualizacao.empty:
                ...
                for i, row in df_pagina_exibicao.iterrows():
                    if str(row.get('STATUS')) == "EM EXECUÇÃO":
                        ...
                        tempo_formatado = "---"
                        try:
                            # Pegamos o 'agora' local do sistema
                            ...
                            if h_ini_str:
                                ...
                                # Se dt_ini for maior que agora, ele começou ontem (ajuste de 24h)
                                if dt_ini > agora:
...
# Perfil Adm
    ...
    if user_p['PERFIL'] in ['Perfil System', 'Perfil Gerencial Clientes']:
        ...
        # Garanta que a chave da aba seja inicializada no início do Painel Admin
        ...
        escolha_aba_admin = st.radio(
            "Navegação:", 
            opcoes_admin, 
            index=opcoes_admin.index(st.session_state.aba_admin_ativa) if st.session_state.aba_admin_ativa in opcoes_admin else 0,
            key="radio_admin_main",
            horizontal=True,
            label_visibility="collapsed"
        )
        # Atualiza a memória sempre que o rádio for clicado
                    ...
                    with st.container(border=True):
                        ...
                        with c3:
                            ...
                            if st.button("Marcar como resolvido ✅", key=f"btn_sol_{row['ID']}", use_container_width=True):
                                ...
                                st.session_state.aba_admin_ativa = label_pedidos
                                st.rerun()
